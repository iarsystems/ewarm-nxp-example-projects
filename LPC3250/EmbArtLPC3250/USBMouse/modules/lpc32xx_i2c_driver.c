/***********************************************************************
 * $Id: lpc32xx_i2c_driver.c 28 2014-01-27 14:45:13Z danielru $
 *
...
void enable_i2c_irq_int(UNS_32 devid)
{
  if ((I2C_CFG_T *) devid == &i2cdat[0])
    SIC1_IntEnable(Sub1I2C_1_INT, ENABLE_IRQ);
  else if ((I2C_CFG_T *) devid == &i2cdat[1])
    SIC1_IntEnable(Sub1I2C_2_INT, ENABLE_IRQ);
}
...
{
  if ((I2C_CFG_T *) devid == &i2cdat[0])
    SIC1_IntEnable(Sub1I2C_1_INT, DISABLE_IRQ);
  else if ((I2C_CFG_T *) devid == &i2cdat[1])
    SIC1_IntEnable(Sub1I2C_2_INT, DISABLE_IRQ);
}
...
void i2c_mtxrx_handler(INT_32 devid)
{
  ...
  /* STOP generated and Receive buffer empty? */
  else if (((device->regptr->i2c_stat & I2C_TDI) != 0) && ((device->regptr->i2c_stat & I2C_RFE) != 0) )
  {
    if (setup->rx_data[0] != 0x3F)
    {
      asm("nop");
    }
 ...
 *
 **********************************************************************/
STATUS_T i2c_close(INT_32 devid)
{
  UNS_32 i2c_id = 9999;
  STATUS_T status = _ERROR;
 ...
 *
 **********************************************************************/
STATUS_T i2c_ioctl(INT_32 devid,
                 INT_32 cmd,
                 INT_32 arg)
{
  UNS_32 tmp, high, low;
