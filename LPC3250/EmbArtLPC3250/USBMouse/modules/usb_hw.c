...
/*************************************************************************
 * Function Name: isp1301_write
 * Parameters:  Int8U reg, Int8U val
 *
 * Return: none
 *
 * Description: For sending commands to the ISP1301 chip
 *
 *************************************************************************/
static
void isp1301_write(Int8U reg, Int8U val)
{
  I2C_STS_bit.TDI = 1;     // clear transaction done flag
  //I2C_RX = 1;
...
Int32U Save;
  ENTR_CRT_SECTION(Save);
  isp1301_write(Conn?0x006:0x007, // Send OTG Control (Clear/Set) register address
                0x01);            // Clear DP_PULLUP bit
  EXT_CRT_SECTION(Save);
}
...
#if 0 != USB_DEV_PRIORITY
  if (SIC1_RSR_bit.USB_dev_hp_int)
  {
...
#if USB_DMA_DD_MAX_NUMB > 0
  if (SIC1_RSR_bit.USB_dev_dma_in)
  {
    Int32U UsbDmaInt = 0;
    Int32U Tmp;
