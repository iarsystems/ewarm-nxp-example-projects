 ...
 * Return: none
 *
 * Description: Touchscreen end of convertion interrupt handler
 *
 *************************************************************************/
...
Int32U Temp1, Temp2, Detect;
  ...
  if(TouchScrState == TOUCH_SCR_PEN_DETECTION)
  {
    ...
    // Disable periodic update of positiuon of the pen
    TouchScrState = TOUCH_SCR_PEN_DETECTION;
    ADC_GC_bit.SSM = 1;
...
volatile Int32U Dummy;
  // Init variable
  TouchScrState = TOUCH_SCR_PEN_DETECTION;
  // Assign GPIO to ADC module
  // AN0(PJ0) - TouchBotton, AN1(PJ2) - TouchTop
  // AN2(PJ4) - TouchLeft,   AN3(PJ7) - TouchRight
  IOCON_ADC_MUX &= ~((1UL << 0) | (1UL << 2) | \
                     (1UL << 4) | (1UL << 7));
  ...
  // Enable interrupts
  ADC_IM_bit.PMSK   = 1;      // Pen Interrupt enable
  ADC_IM_bit.EOSMSK = 1;      // End-of-Sequence Interrupt enable
  ...
  VIC_SetVectoredIRQ(TouchScrPen_Handler,PenTouchSlot,VIC_ADC_PENIRQ);
  VIC_EnableInt(1UL << VIC_ADC_PENIRQ);
  VIC_SetVectoredIRQ(TouchScrEOC_Handler,EndOfConvSlot,VIC_ADC_TSCIRQ);
  VIC_EnableInt(1UL << VIC_ADC_TSCIRQ);
...
void TouchScrStartMesure (void)
{
  ...
  {
    ADC_GC_bit.SSB = 1; // Start convertion
  }
}
