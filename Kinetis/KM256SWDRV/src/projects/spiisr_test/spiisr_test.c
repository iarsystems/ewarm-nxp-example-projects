...
/* callback function                                                          */
static void spi1_callback (SPI_CALLBACK_SRC module, SPI_CALLBACK_TYPE type);
...
void main (void)
{
  ...
  SPI_Init  (SPI1, SPI_MODULE_DIV2_8B_IRQ_CONFIG, NULL);
  SPI_InstallCallback (PRI_LVL1, spi1_callback);
  ...
  SPI_TxIsrEnable(SPI1);  /* start the 1st packet with 1 Tx byte, no Rx byte  */

  spi_tx_cnt = 1; spi_rx_cnt = 1; spiBuff[0]=0x05; spi_cnt=0;
  SPI_TxIsrEnable(SPI1);  /* start the 2nd packet with 1 Tx byte, 1 Rx byte   */

  spi_tx_cnt = 2; spi_rx_cnt = 8; spiBuff[0]=0x03; spiBuff[1]=0; spi_cnt=0;
  SPI_TxIsrEnable(SPI1);  /* start the 3rd packet with 2 Tx bytes, 8 Rx bytes */
  ...
  if (module == SPI1_CALLBACK)
  {
    if (type == SPI_TX_CALLBACK)        /* TX IRQ                             */
    {
      if (spi_tx_cnt) {                 /* Tx state?                          */
        GPIO_Clr  (GPIOF,SPI1_SS );     /* managing the slave select          */
        SPI_PutByte(SPI1,spiBuff[spi_cnt]);   /* sending the data             */
        spi_cnt++;
        ...
        if (spi_rx_cnt) {               /* Rx state?                          */
          ...
          SPI_PutByte(SPI1,0xFF);       /* dummy Tx data in case of reading   */
        }
      }
    }
    ...
    {
      register uint8 temp;
      temp = SPI_GetByte(SPI1);         /* save received byte to temp         */
      ...
      if (!spi_rx_cnt) {                /* end of receiving?                  */
        SPI_TxIsrDisable(SPI1);         /* disable Tx IRQ                     */
        GPIO_Set  (GPIOF,SPI1_SS );     /* managing the slave select          */
      }
    }
