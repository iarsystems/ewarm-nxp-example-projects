...
void diagnostic_setup (void)
{
  /* Route AFE_CH0 modulator & clock outputs to PTK7 & PTL2 for monitoring    */
  XBAR_Init (XBAR_MODULE_NO_EDGE_DETECT_CONFIG,
             XBAR_MODULE_NO_EDGE_DETECT_CONFIG,
             XBAR_MODULE_NO_EDGE_DETECT_CONFIG,
             XBAR_MODULE_NO_EDGE_DETECT_CONFIG, PRI_LVL0, NULL);
  PORT_Init (PORTK,PORT_MODULE_ALT3_MODE,PIN7);
  PORT_Init (PORTL,PORT_MODULE_ALT2_MODE,PIN2);
  XBAR_Path (XBAR_AFEMDAT0, XBAR_OUT9);
  XBAR_Path (XBAR_AFEMCLK, XBAR_OUT10);
