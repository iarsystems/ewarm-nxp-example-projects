...
void main (void)
{
  IRTC_Init (IRTC_MODULE_COMP_OFF_CONFIG,
             ...
             IRTC_TAMPER_PIN_POL_HIGH_CONFIG(TAMPER_FILT_CLK_64HZ,10),
             IRTC_TAMPER_PIN_DI_CONFIG,
             IRTC_TAMPER0_MASK|IRTC_ALM_MASK|IRTC_1HZ_MASK);
...
static void irtc_callback (IRTC_CALLBACK_TYPE type, void *data)
{
  ...
  if(type == IRTC_TAMPER0_CALLBACK) { memcpy (&log,data,sizeof(tTAMPER_LOG)); }
}
