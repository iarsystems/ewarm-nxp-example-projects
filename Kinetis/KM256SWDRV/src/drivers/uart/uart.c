...
#include "uart.h"
  ...
  #error "The uart.c module is specific to the MKM34Z256VLx7 microcontrollers."
#endif

/******************************************************************************
 * data type definitions                                                      *
 ******************************************************************************/
static UART_CALLBACK pCallbackUARTx;

/******************************************************************************
 * public function definitions                                                *
 ******************************************************************************/
void UART_InstallCallback (uint8 ip, UART_CALLBACK pCallback)
{
  if (pCallback) { pCallbackUARTx = pCallback; NVIC_SetIsr(INT_UART0_UART1_UART2_UART3,ip); }
}
...
weak void uartx_isr (void)
{
  ...
  if (SIM_SCGC4 & SIM_SCGC4_UART1_MASK)
  {
    ...
    if(tmp != 0)
    {
      ...
      pCallbackUARTx (UART1_CALLBACK, UART_EDG_CALLBACK, tmp);
    }

    tmp = UART1_S1 & (UART1_C2 & (UART_C2_RIE_MASK|UART_C2_TCIE_MASK|         \
                                  UART_C2_TIE_MASK));
    if(tmp & UART_S1_RDRF_MASK)
      pCallbackUARTx (UART1_CALLBACK, UART_RX_CALLBACK, tmp);

    if(tmp & (UART_S1_TC_MASK|UART_S1_TDRE_MASK))
      pCallbackUARTx (UART1_CALLBACK, UART_TX_CALLBACK, tmp);

    tmp = UART1_S1 & (UART1_C3 & (UART_C3_PEIE_MASK|UART_C3_FEIE_MASK|        \
                                  UART_C3_NEIE_MASK|UART_C3_ORIE_MASK));
    if(tmp != 0)
      pCallbackUARTx (UART1_CALLBACK, UART_ER_CALLBACK, tmp);
  }
  ...
  if (SIM_SCGC4 & SIM_SCGC4_UART2_MASK)
  {
    ...
    if(tmp != 0)
    {
      ...
      pCallbackUARTx (UART2_CALLBACK, UART_EDG_CALLBACK, tmp);
    }

    tmp = UART2_S1 & (UART2_C2 & (UART_C2_RIE_MASK|UART_C2_TCIE_MASK|         \
                                  UART_C2_TIE_MASK));
    if(tmp & UART_S1_RDRF_MASK)
      pCallbackUARTx (UART2_CALLBACK, UART_RX_CALLBACK, tmp);

    if(tmp & (UART_S1_TC_MASK|UART_S1_TDRE_MASK))
      pCallbackUARTx (UART2_CALLBACK, UART_TX_CALLBACK, tmp);

    tmp = UART2_S1 & (UART2_C3 & (UART_C3_PEIE_MASK|UART_C3_FEIE_MASK|        \
                                  UART_C3_NEIE_MASK|UART_C3_ORIE_MASK));
    if(tmp != 0)
      pCallbackUARTx (UART2_CALLBACK, UART_ER_CALLBACK, tmp);
  }
  ...
  if (SIM_SCGC4 & SIM_SCGC4_UART3_MASK)
  {
    ...
    if(tmp != 0)
    {
      ...
      pCallbackUARTx (UART3_CALLBACK, UART_EDG_CALLBACK, tmp);
    }

    tmp = UART3_S1 & (UART3_C2 & (UART_C2_RIE_MASK|UART_C2_TCIE_MASK|         \
                                  UART_C2_TIE_MASK));
    if(tmp & UART_S1_RDRF_MASK)
      pCallbackUARTx (UART3_CALLBACK, UART_RX_CALLBACK, tmp);

    if(tmp & (UART_S1_TC_MASK|UART_S1_TDRE_MASK))
      pCallbackUARTx (UART3_CALLBACK ,UART_TX_CALLBACK, tmp);

    tmp = UART3_S1 & (UART3_C3 & (UART_C3_PEIE_MASK|UART_C3_FEIE_MASK|        \
                                  UART_C3_NEIE_MASK|UART_C3_ORIE_MASK));
    if(tmp != 0)
      pCallbackUARTx (UART3_CALLBACK, UART_ER_CALLBACK, tmp);
  }
}
/******************************************************************************
 * End of module                                                              *
