...
#include "gpio.h"
  ...
  #error "The gpio.c module is specific to the MKM34Z256VLx7 microcontrollers."
#endif
...
void PORTJ_Init (tPORT port, uint8 pin_mask)
{
  register uint16 i;

  for (i=0; i<8; i++)
    if(pin_mask & (uint8)(1u << i)) { PORTJ_BASE_PTR->PCR[i] = port.PCR; }
  PORTJ_ISFR = pin_mask;
}

void PORTK_Init (tPORT port, uint8 pin_mask)
{
  register uint16 i;

  for (i=0; i<8; i++)
    if(pin_mask & (uint8)(1u << i)) { PORTK_BASE_PTR->PCR[i] = port.PCR; }
  PORTK_ISFR = pin_mask;
}

void PORTL_Init (tPORT port, uint8 pin_mask)
{
  register uint16 i;

  for (i=0; i<8; i++)
    if(pin_mask & (uint8)(1u << i)) { PORTL_BASE_PTR->PCR[i] = port.PCR; }
  PORTL_ISFR = pin_mask;
}

void PORTM_Init (tPORT port, uint8 pin_mask)
{
  register uint16 i;

  for (i=0; i<8; i++)
    if(pin_mask & (uint8)(1u << i)) { PORTM_BASE_PTR->PCR[i] = port.PCR; }
  PORTM_ISFR = pin_mask;
}
/******************************************************************************
...
weak void portx_isr (void)
{
  ...
  /* process interrupt callback function for PORTJ                            */
  if (SIM_SCGC6 & SIM_SCGC6_PORTJ_MASK)
  {
    PORTJ_ISFR = tmp = (uint8)PORTJ_ISFR;
    if (tmp != 0) { pCallbackPTx (PORTJ_CALLBACK, tmp); }
  }

  /* process interrupt callback function for PORTK                            */
  if (SIM_SCGC6 & SIM_SCGC6_PORTK_MASK)
  {
    PORTK_ISFR = tmp = (uint8)PORTK_ISFR; /* read and clear interrupt flags   */
    if (tmp != 0) { pCallbackPTx (PORTK_CALLBACK, tmp); }
  }

  /* process interrupt callback function for PORTL                            */
  if (SIM_SCGC6 & SIM_SCGC6_PORTL_MASK)
  {
    PORTL_ISFR = tmp = (uint8)PORTL_ISFR; /* read and clear interrupt flags   */
    if (tmp != 0) { pCallbackPTx (PORTL_CALLBACK, tmp); }
  }

  /* process interrupt callback function for PORTM                            */
  if (SIM_SCGC6 & SIM_SCGC6_PORTM_MASK)
  {
    PORTM_ISFR = tmp = (uint8)PORTM_ISFR; /* read and clear interrupt flags   */
    if (tmp != 0) { pCallbackPTx (PORTM_CALLBACK, tmp); }
  }
}
/******************************************************************************
 * End of module                                                              *
