...
/******************************************************************************
* Possible values for the secondary count source used by @ref TMR_Init() function
*
...
#define TMRCH0_SetScsFilter(cnt,per)                                          \
                TMR0_FILT = TMR_FILT_FILT_CNT((cnt))|TMR_FILT_FILT_PER((per))
#define TMRCH1_SetScsFilter(cnt,per)                                          \
                TMR1_FILT = TMR_FILT_FILT_CNT((cnt))|TMR_FILT_FILT_PER((per))
#define TMRCH2_SetScsFilter(cnt,per)                                          \
                TMR2_FILT = TMR_FILT_FILT_CNT((cnt))|TMR_FILT_FILT_PER((per))
#define TMRCH3_SetScsFilter(cnt,per)                                          \
                TMR3_FILT = TMR_FILT_FILT_CNT((cnt))|TMR_FILT_FILT_PER((per))
/***************************************************************************//*!
 * @brief   Configures quad timer channel secondary count source input filter.
 * @details This macro configures quad timer channel secondary count source input
            filter.
 * @param   ch    CH0,CH1,CH2,CH3.
 * @param   cnt   Number of consecutive samples from 0 to 7 that must agree prior
 *                to the input filter accepting an input transition. A value
 *                of 0 represents 3 samples and value of 7 represents 10 samples.
 * @param   per   @ref uint8 filter sampling period. Each input is sampled multiple
 *                times at the rate specified by this parameter. If per is 0
 *                (default), then the input filter is bypassed.
 * @note    Implemented as an inline macro.
 * @see     @ref TMR_Init, @ref SIM_SelTmrScs
 * @warning When changing values for filter sampling period from one non-zero
 *          value to another non-zero value, write a value of zero first to clear
 *          the filter.
 ******************************************************************************/
#define TMR_SetScsFilter(ch,cnt,per)   TMR##ch##_SetScsFilter(cnt,per)
