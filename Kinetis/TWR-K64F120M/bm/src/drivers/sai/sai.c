 ...
 * setting:		which pin mux setting used, please refer to above pin mux
 *				settings available on 256BGA package for K70
 */
void sai_pinmux_init(unsigned char port, unsigned char setting)
{
	if (port == 0) {
		if (setting == 0) {
			/*
			 * using PORTC pins
			 * PTC3(ALT6), I2S0_TX_BCLK	-	J37 pin2
			 * PTC3(ALT6), I2S0_TX_BCLK	-	J37 pin2
			 * PTC2(ALT6), I2S0_TX_FS	-	J51 pin11 or 14
			 * PTC9(ALT4), I2S0_RX_BCLK	-	J44 pin5
			 * PTC10(ALT4), I2S0_RX_FS	-	J44 pin8
			 * PTC5(ALT4), I2S0_RXD0	-	J37 pin8
			 * PTC11(ALT4), I2S0_RXD1	-	J44 pin11
			 * PTC1(ALT6), I2S0_TXD0	-	J51 pin5 or 8
			 * PTC0(ALT6), I2S0_TXD1	-	J51 pin2
			 * PTC6(ALT6), I2S0_MCLK	-	J37 pin11
			 */
			PORTC_PCR(3) = PORT_PCR_MUX(6);
			PORTC_PCR(2) = PORT_PCR_MUX(6);
			PORTC_PCR(9) = PORT_PCR_MUX(4);
			PORTC_PCR(10) = PORT_PCR_MUX(4);
			PORTC_PCR(5) = PORT_PCR_MUX(4);
			PORTC_PCR(11) = PORT_PCR_MUX(4);
			PORTC_PCR(1) = PORT_PCR_MUX(6);
			PORTC_PCR(0) = PORT_PCR_MUX(6);
			PORTC_PCR(6) = PORT_PCR_MUX(6);
		} else if (setting == 1) {
			/*
			 * using PORTA pins
			 * PTA5(ALT6), I2S0_TX_BCLK	-	J35 pin8
			 * PTA13(ALT6), I2S0_TX_FS	-	J35 pin14
			 * PTC6(ALT4), I2S0_RX_BCLK	-	J37 pin11
			 * PTC7(ALT4), I2S0_RX_FS	-	J37 pin14
			 * PTC5(ALT4), I2S0_RXD0
			 * PTC11(ALT4), I2S0_RXD1
			 * PTA12(ALT6), I2S0_TXD0	-	J35 pin11
			 * PTC0(ALT6), I2S0_TXD1	-
			 * PTC8(ALT4), I2S0_MCLK	-	J44 pin2
			 */
			PORTA_PCR(5) = PORT_PCR_MUX(6);
			PORTA_PCR(13) = PORT_PCR_MUX(6);
			PORTC_PCR(6) = PORT_PCR_MUX(4);
			PORTC_PCR(7) = PORT_PCR_MUX(4);
			PORTC_PCR(5) = PORT_PCR_MUX(4);
			PORTC_PCR(11) = PORT_PCR_MUX(4);
			PORTA_PCR(12) = PORT_PCR_MUX(6);
			PORTC_PCR(0) = PORT_PCR_MUX(6);
			PORTC_PCR(8) = PORT_PCR_MUX(4);
		}
	} else if (port == 1) {
...
void sai_stop_dbg_enable(unsigned char port, unsigned char tx,
...
void sai_soft_reset(unsigned char port, unsigned char tx)
{
	if (port == 0) {
		if (tx) {
			/* software reset */
			I2S_TCSR_REG(I2S0_BASE_PTR) |= I2S_TCSR_SR_MASK;
			/* clear software reset bit since it's sticky */
			I2S_TCSR_REG(I2S0_BASE_PTR) &= ~I2S_TCSR_SR_MASK;
		} else {
			/* software reset */
			I2S_RCSR_REG(I2S0_BASE_PTR) |= I2S_RCSR_SR_MASK;
			/* clear software reset bit since it's sticky */
			I2S_RCSR_REG(I2S0_BASE_PTR) &= ~I2S_RCSR_SR_MASK;
		}
	} else if (port == 1) {
...
/* Input parameter */
void sai_watermark_config(unsigned char port, unsigned char tx,
 ...
 * bci:		bit clock input
 */
void sai_mode_config(unsigned char port, unsigned char tx, unsigned char mode,
					 unsigned char bcs, unsigned char bci)
{
	if (port == 0) {
		if (tx) {
			/*configure SAI synchronous mode */
			I2S_TCR2_REG(I2S0_BASE_PTR) |= I2S_TCR2_SYNC(mode);
 ...
 * port:	SAI module used
 * tx:		transmit or receive
 * mclksel:	select master clock to generate internal bit clock,
 *			ignored when configured for external generated bit clock
 * bclkpol:	bit clock polarity
 * bclkdir:	bit clock direction
 * bclkdiv:	bit clock divide, division value is (DIV+1)*2
 */
void sai_bclk_config(unsigned char port, unsigned char tx,
					 unsigned char mclksel, unsigned char bclkpol,
					 unsigned char bclkdir, unsigned char bclkdiv)
{
	if (port == 0) {			/* SAI0 init */
		if (tx) {
			...
			/*
			 * master clock select
			 * divide down master clock to generate bit clock
			 */
			I2S_TCR2_REG(I2S0_BASE_PTR) |= I2S_TCR2_MSEL(mclksel)
				| I2S_TCR2_DIV(bclkdiv)
				;
			if (bclkpol)
			...
			/* clear master clock select */
			I2S_RCR2_REG(I2S0_BASE_PTR) &= ~I2S_RCR2_MSEL_MASK;
			/* divide down master clock to generate bit clock */
			I2S_RCR2_REG(I2S0_BASE_PTR) &= ~I2S_RCR2_DIV_MASK;

			/*
			 * master clock select
			 * divide down master clock to generate bit clock
			 */
			I2S_RCR2_REG(I2S0_BASE_PTR) |= I2S_RCR2_MSEL(mclksel)
				| I2S_RCR2_DIV(bclkdiv)
				;
			if (bclkpol)
 ...
 * port:	SAI module used
 * tx:		transmit or receive
 * chen:	channel enable, 0x01 enable channel one,
 *			0x02 enable channel two, 0x03 enable both channel
 */
void sai_channel_enable(unsigned char port, unsigned char tx,
						unsigned char chen)
{
	if (port == 0) {
		if (tx)
 ...
 * port:	SAI module used
 * tx:		transmit or receive
 * chen:	channel disable, 0x01 disable channel one,
 *			0x02 disable channel two, 0x03 disable both channel
 */
void sai_channel_disable(unsigned char port, unsigned char tx,
						 unsigned char chen)
{
	if (port == 0) {
		if (tx)
 ...
 *	port:	SAI module used
 *	tx:		transmit or receive
 *	wdfl:	configure which word the start of flag is set,
 *			should be configured less than frame size
 */
void sai_word_flg_config(unsigned char port, unsigned char tx,
						 unsigned char wdfl)
{
	if (port == 0) {
		if (tx)
 ...
 * port:		SAI module used
 * tx:			transmit or receive
 * framesize:	configure number of words in each frame, maximum supported
 *				frame size is 32
 * sywd:		configure length of the frame sync in number of bit clocks
 * mf:			specify LSB or MSB first
 */
void sai_frame_config(unsigned char port, unsigned char tx,
					  unsigned char framesize, unsigned char sywd,
					  unsigned char mf)
{
	if (port == 0) {
		if (tx) {
			...
			/*
			 * configure number of words in each frame
			 * configure length of the frame sync in number of bit clocks
			 */
			I2S_TCR4_REG(I2S0_BASE_PTR) |= I2S_TCR4_FRSZ(framesize)
				| I2S_TCR4_SYWD(sywd)
				;
			if (mf)
			 ...
			 * configure length of the frame sync in number of bit clocks
			 */
			I2S_RCR4_REG(I2S0_BASE_PTR) |= I2S_RCR4_FRSZ(framesize)
				| I2S_RCR4_SYWD(sywd)
				;
			if (mf)
 ...
 */
void sai_frameclk_config(unsigned char port, unsigned char tx,
						 unsigned char fse, unsigned char fsp,
						 unsigned char fsd)
{
	if (port == 0) {
		if (tx) {
 ...
 *	wnw:	word N width
 *	w0w:	word 0 width
 *	fbt:	configure the bit index of the first bit transmitted for
 *			each word in the frame
 */
void sai_wordlength_config(unsigned char port, unsigned char tx,
						   unsigned char wnw, unsigned char w0w,
						   unsigned char fbt)
{
	if (port == 0) {
		if (tx) {
			...
			/*
			 * configure number of bits in each word except first in frame
			 * configure number of bits for first word in each frame
			 * configure bit index of first bit transmitted of each word
			 */
			I2S_TCR5_REG(I2S0_BASE_PTR) |= I2S_TCR5_WNW(wnw) |
										   I2S_TCR5_W0W(w0w) |
										   I2S_TCR5_FBT(fbt);
		} else {
			 ...
			 * configure bit index of first bit transmitted of each word
			 */
			I2S_RCR5_REG(I2S0_BASE_PTR) |= I2S_RCR5_WNW(wnw) |
										   I2S_RCR5_W0W(w0w) |
										   I2S_RCR5_FBT(fbt);
		}
	} else if (port == 1) {
...
unsigned char sai_tx(unsigned char port, unsigned char channel,
					 unsigned int txdata)
{
...
unsigned char sai_rx(unsigned char port, unsigned char channel,
					 unsigned int *rxdata)
{
...
void sai_rd_rdr(unsigned char port,
...
unsigned char sai_tx_fifo_status(unsigned char port, unsigned char channel)
{
	...
	if (port == 0) {
		wr_fp =
			(I2S_TFR_REG(I2S0_BASE_PTR, channel) & I2S_TFR_WFP_MASK) >>
			I2S_TFR_WFP_SHIFT;
		rd_fp =
			(I2S_TFR_REG(I2S0_BASE_PTR, channel) & I2S_TFR_RFP_MASK) >>
			I2S_TFR_RFP_SHIFT;
		if (wr_fp == rd_fp)
			return FIFO_EMPTY;
...
unsigned char sai_rx_fifo_status(unsigned char port, unsigned char channel)
{
	...
	if (port == 0) {
		wr_fp =
			(I2S_RFR_REG(I2S0_BASE_PTR, channel) & I2S_RFR_WFP_MASK) >>
			I2S_RFR_WFP_SHIFT;
		rd_fp =
			(I2S_RFR_REG(I2S0_BASE_PTR, channel) & I2S_RFR_RFP_MASK) >>
			I2S_RFR_RFP_SHIFT;
		if (wr_fp == rd_fp)
			return FIFO_EMPTY;
		else if ((wr_fp & 0x08) != (rd_fp & 0x08))
 ...
 *	port:	SAI module used
 *	tx:		transmit or receive
 *	mask:	mask for each word in frame, if any bit in mask is set,
 *			then that word is tristated
 */
...
void sai_mclk_config(unsigned char port, unsigned char mics, unsigned char moe,
					 unsigned char fract, unsigned char divide)
{
	if (port == 0) {
		/*
		...
		/* MCLK divider input clock select */
		I2S_MCR_REG(I2S0_BASE_PTR) |= I2S_MCR_MICS(mics);
		...
		/*
		 * Please note that MICS field cannot be changed once MCLK divider
		 * enabled, because MOE bit will enable MCLK divider and configure
		 * MCLK pin as output, so MOE bit needs to be set after setting MICS,
		 * otherwise the change to MICS bit will not be valid!!!
		 */
		if (moe)
...
void sai_clear_reg(unsigned char port)
{
	if (port == 0) {
		...
		/*
		 * soft reset, reset internal tranmit logic and FIFO pointers,
		 * this operation will clear TDR and TFR, don't try to write 0
		 * to TDR for reg clear, any write to TDR will increment TFR
		 */
		...
		/*
		 * soft reset, reset internal receive logic and FIFO pointers,
		 * this operation will clear RDR and RFR, don't try to write 0
		 * to RDR for reg clear, any write to RDR will increment RFR
		 */
		...
		/*
		 * Clearing MCR register is a bit tricky, first disable MOE bit,
		 * then clear MDR, then change MICS to 0x0
		 */
...
void sai_monitor_fifo_status(unsigned char port, unsigned char channel)
{
	if (port == 0) {
		printf("I2S_TFR_REG(I2S0_BASE_PTR,%d) is 0x%x\n", channel,
			   I2S_TFR_REG(I2S0_BASE_PTR, channel));
		printf("I2S_RFR_REG(I2S0_BASE_PTR,%d) is 0x%x\n", channel,
			   I2S_RFR_REG(I2S0_BASE_PTR, channel));
	} else if (port == 1) {
		printf("Error: Port %d is not available yet.", port);
	}
}
	...
	if (port == 0) {
		if (tx) {
			...
			if (I2S_TCSR_REG(I2S0_BASE_PTR) & I2S_TCSR_WSF_MASK) {
				...
				/*
				 * indicate error in the externally generated frame sync
				 * has been detected
				 */
				...
				/*
				 * indicates number of words in an enabled transmit FIFO is
			...
			if (I2S_RCSR_REG(I2S0_BASE_PTR) & I2S_RCSR_WSF_MASK) {
				...
				/*
				 * indicate error in the externally generated frame sync has
				 * been detected
				 */
				...
				/*
				 * indicates number of words in an enabled receive FIFO is
...
{
	unsigned char cnt;
	unsigned int tcsr = I2S_TCSR_REG(I2S0_BASE_PTR);	/* transmit control */
	unsigned int tfr0 = I2S_TFR_REG(I2S0_BASE_PTR, 0);	/* channel 0 tx FIFO */
	unsigned int tfr1 = I2S_TFR_REG(I2S0_BASE_PTR, 1);	/* channel 1 tx FIFO */
...
#pragma diag_suppress=Pe550
	...
	if (tcsr & I2S_TCSR_WSF_MASK) {
		...
		/*
		 * indicates number of words in an enabled transmit FIFO is less than
		 * or equal to the transmit FIFO watermark
		 */
		I2S_TCSR_REG(I2S0_BASE_PTR) &= ~I2S_TCSR_FRF_MASK;	/*clear flag */
...
void sai0_rx_isr(void)
{
	...
	if (rcsr & I2S_RCSR_WSF_MASK) {
		...
		/*
		 * indicates number of words in an enabled receive FIFO is greater
