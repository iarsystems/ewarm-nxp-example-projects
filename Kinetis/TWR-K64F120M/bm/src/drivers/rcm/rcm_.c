...
void outSRS(void)
{
	...
	if (RCM_SRS0 & RCM_SRS0_WAKEUP_MASK) {
		...
		if (((SMC_PMCTRL & SMC_PMCTRL_STOPM_MASK) == 4) &&
			((SMC_VLLSCTRL & SMC_VLLSCTRL_VLLSM_MASK) == 0))
...
void outsrs(void)
{
	...
	GPIOE_PCOR = 0x04000000;				/* set Port E 26 indicate wakeup
											 * - clears it in llwu_isr */
	/*used for wakeup testing */
	PORTC_PCR19 = (PORT_PCR_MUX(01) |		/* GPIO */
				   PORT_PCR_IRQC(0x00) |	/* falling edge enable */
				   PORT_PCR_DSE_MASK);		/* enable high drive strength */
	GPIOC_PDDR |= 0x00080000;				/* set Port C 19 as output */
	GPIOC_PSOR = 0x00080000;				/* set Port C 19 SRS output
											 * outsrs uses this pin */
	...
	/* set Port C 19 SRS output outsrs uses this pin */
	GPIOC_PCOR = 0x00080000;
