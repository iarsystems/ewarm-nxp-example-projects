...
void USB_Init(void)
{
	...
	/*
	 * MPU Configuration
	 * MPU is disable. All accesses from all bus masters are allowed
	 */
	MPU_CESR = 0;
...
void USB_StdReq_Handler(void)
{
	...
	case mGET_DESC:
		...
		case mCONFIGURATION:
			EP_IN_Transfer(EP0,
						   (uint8 *) Configuration_Descriptor,
						   sizeof(Configuration_Descriptor));
			break;

		case mSTRING:
			EP_IN_Transfer(EP0,
						   (uint8 *) String_Table[Setup_Pkt->wValue_l],
...
void USB_Handler(void)
{
	...
	if (u8EndPoint) {
		...
		if (u8IN)
			...
			if (tBDTtable[bEP0OUT_ODD].Stat.RecPid.PID ==
			    mSETUP_TOKEN)
				USB_Setup_Handler();
			else
				USB_EP0_OUT_Handler();
