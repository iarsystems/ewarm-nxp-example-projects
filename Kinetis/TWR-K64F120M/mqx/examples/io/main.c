...
#error This application requires BSPCFG_ENABLE_TTYF defined non-zero in user_config.h. Please recompile BSP with this option.
#endif
   ...
   )
{
   ...
   if (!strcmp("ttyf:", BSP_DEFAULT_IO_CHANNEL))
     fh_ptr = (void *)fopen("ttyb:", BSP_DEFAULT_IO_OPEN_MODE);
   else
     fh_ptr = (void *)fopen("ttyf:", BSP_DEFAULT_IO_OPEN_MODE);
   if (fh_ptr == NULL) {
