...
DESCRIPTION
===========
  This example is a simple demonstration of MQX task synchronization
  using the semaphore object

  There is are three tasks. The first task initializes three semaphores,
  creates NUM_WRITERS write_task's, and creates one read_task.

  The write task opens a connection to all three
  semaphores then waits for sem.write and sem.index. It writes one entry
  in the data array and posts sem.index and sem.read.

  The read task opens a connection to all three semaphores, then 
