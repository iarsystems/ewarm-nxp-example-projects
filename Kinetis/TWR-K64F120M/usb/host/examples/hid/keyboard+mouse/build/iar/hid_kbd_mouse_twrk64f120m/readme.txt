...
DESCRIPTION
===========
  This is an example of device drivers for a Mouse device and Keyboard.

  The program queues transfers on Interrupt USB pipe and waits till the
  data comes back. It prints the data and queues another transfer on
  the same pipe.

  Note that a real keyboard driver also needs to distinguish between
  intentionally repeated and unrepeated key presses. This example simply
  demonstrates how to receive data from a USB Keyboard. 
...
===============
  ...
  4) Connect a mouse or a keyboard to the K64_USB port. An adapter from
  micro-USB to USB-A may be needed.
