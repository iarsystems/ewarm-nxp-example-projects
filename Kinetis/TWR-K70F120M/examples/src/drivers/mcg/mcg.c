 ...
 * If in PEE, BLPI, PBE, FBI, FBE or FEE modes, first return to FEI then configure again.
 */
...
int pll_init(unsigned char init_osc, unsigned char osc_select, int crystal_val, unsigned char hgo_val, unsigned char erefs_val, unsigned char pll_select, signed char prdiv_val, signed char vdiv_val, unsigned char mcgout_select)
{
  ...
  if (mcgout_select)
  {
    ...
    {
      // if not in FEI mode try to return in FEI
      // assume that the device is in PEE, BLPI, PBE, FBI, FBE or FEE mode 
      // check if in PEE mode
      mcg_mode = what_mcg_mode(); // get present MCG mode
      if (mcg_mode == PEE)
      {
        // return to PBE mode
        // MCG_C6_PLLS is already set
        // MCG_C1_IREFS already 0
        // set MCG_C2_LP to 0
        temp_reg = MCG_C2;
        temp_reg &= ~MCG_C2_LP_MASK;
        MCG_C2 = temp_reg;
        // set MCG_C1_CLKS to 2
        temp_reg = MCG_C1;
        temp_reg &= ~MCG_C1_CLKS_MASK;
        temp_reg |= MCG_C1_CLKS(2);
        MCG_C1 = temp_reg;
        ...
        if (erefs_val)
        {
          ...
          if (!(MCG_S & MCG_S_OSCINIT0_MASK)) return 0xF0; // check bit is really set and return with error if not set
        }                
        ...
        if (((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x2) return 0xF1; // check EXT CLK is really selected and return with error if not
        // now in PBE mode
      }
      
      // check if in PBE or BLPE mode      
      mcg_mode = what_mcg_mode(); // get present MCG mode
      if ((mcg_mode == PBE) || (mcg_mode == BLPE))
      {
        // return to FBE mode  
        // MCG_C2_LP is already 0
        // MCG_C1_IREFS is already 0
        // MCG_C1_CLKS already 2
        // set MCG_C6_PLLS to 0
        temp_reg = MCG_C6;
        temp_reg &= ~MCG_C6_PLLS_MASK;
        MCG_C6 = temp_reg;
        // now we need to set FRDIV value...
        ...
        MCG_C1 = temp_reg; 
        // Wait for PLL status bit to clear
        for (i = 0; i < 2000; i++)
        {
          if (!(MCG_S & MCG_S_PLLST_MASK)) break;
        }
        if (MCG_S & MCG_S_PLLST_MASK) return 0xF3;
        // now in FBE mode
      }
      
      // check if in FBI, FBE or FEE mode      
      mcg_mode = what_mcg_mode(); // get present MCG mode
      if ((mcg_mode == FBI) || (mcg_mode == FBE) || (mcg_mode == FEE))
      {
        // return to FEI mode
        // MCG_C6_PLLS is already 0
        // set MCG_C1_IREFS to 1
        // set MCG_C1_CLKS to 0
        temp_reg = MCG_C1;
        temp_reg &= ~(MCG_C1_CLKS_MASK | MCG_C1_IREFS_MASK); // Clear values in these fields
        temp_reg |= MCG_C1_CLKS(0) | MCG_C1_IREFS_MASK; // Set the required CLKS value and IREFS bit
        MCG_C1 = temp_reg;  
        ...
        if (!(MCG_S & MCG_S_IREFST_MASK)) return 0xF5; // check bit is really clear and return with error if not set
        ...
        if (((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x0) return 0xF7; // check EXT CLK is really selected and return with error if not 
        // now in FEI mode                                          
      }
                
     // check again if not in FEI mode
     if (!((((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x0) && // check CLKS mux has selcted FLL output
        (MCG_S & MCG_S_IREFST_MASK) &&                                   // check FLL ref is internal ref clk
        (!(MCG_S & MCG_S_PLLST_MASK))))                                 // check PLLS mux has selected FLL
  ...
  if (init_osc)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        // This has the affect of sometimes preventing the oscillator from running. To keep the oscillator amplitude
        // low, RANGE = 2 should not be used. This should be removed when fixed silicon is available.
        //temp_reg |= MCG_C10_RANGE1(2) | (hgo_val << MCG_C10_HGO1_SHIFT) | (erefs_val << MCG_C10_EREFS1_SHIFT);
        temp_reg |= MCG_C10_RANGE1(1) | (hgo_val << MCG_C10_HGO1_SHIFT) | (erefs_val << MCG_C10_EREFS1_SHIFT);
      }
      MCG_C10 = temp_reg;
    } // if (!osc_select)
...
// Check PLL output frequency is within spec.
  ...
  // The loss of lock interrupt can be enabled by seperately OR'ing in the LOLIE bit in MCG_C12
    ...
    temp_reg &= ~MCG_C12_VDIV1_MASK; // clear VDIV settings
    temp_reg |= MCG_C12_VDIV1(vdiv_val - 16); // write new VDIV
    MCG_C12 = temp_reg; // update MCG_C12
