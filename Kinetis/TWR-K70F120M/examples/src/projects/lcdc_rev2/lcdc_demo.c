...
void lcdc_demo()
{
  ...
  //Set maximum address
  unsigned int max_address=FRAME_START_ADDRESS+FRAME_BUFFER_SIZE+FRAME_BUFFER_SIZE;
  ...
  unsigned int bp_address=FRAME_START_ADDRESS+FRAME_BUFFER_SIZE;
 ...
 //Enable EOF interrupt
  ...
  //Create a graphic window frame buffer of all white
  pointer32=(int *)(FRAME_START_ADDRESS+FRAME_BUFFER_SIZE); //Create buffer in memory directly after the image
  ...
  LCDC_LGWSAR = FRAME_START_ADDRESS+FRAME_BUFFER_SIZE;
  ...
  while(1)
  {
    ...
    //Reset the vairables
    bp_address=FRAME_START_ADDRESS+FRAME_BUFFER_SIZE;
    LCDC_LSSAR=bp_address;
  }
}
