/**********************************************************************
* $Id: usbhw.c 28 2014-01-27 14:45:13Z danielru $		usbhw.c					2011-06-02
*//**
* @file		usbhw.c
...
**********************************************************************/
#include <string.h>
#include "LPC18xx.H"                        /* LPC17xx definitions */
#include "lpc_types.h"
#include "usb.h"
#include "usbhw.h"
#include "usbcfg.h"
...
//	scu_pinmux(0x2,2,MD_PLN,FUNC0);    //  0: usb0 usb0_ind1           1: uart0 uclk0              2: ebi a11                  3: sgpio sgpio_6
//	scu_pinmux(0x2,0,MD_PLN,FUNC0);    //  0: usb0 usb0_pwr_en         1: uart0 txd0               2: ebi a13                  3: sgpio sgpio_4
//	scu_pinmux(0x2,1,MD_PLN,FUNC0);    //  0: usb0 usb0_pwr_fault      1: uart0 rxd0               2: ebi a12                  3: sgpio sgpio_5
//	scu_pinmux(0x8,0,MD_PLN,FUNC0);    //  0: motocon pcap0_2          1: usb0 usb0_pwr_fault      2:  nc                      3: gpio4 gpio4_0
#ifdef USE_USB0
	scu_pinmux(0x8,1,MD_PLN,FUNC1);    //  0: motocon pcap0_1          1: usb0 usb0_ind1           2:  nc                      3: gpio4 gpio4_1
	scu_pinmux(0x8,2,MD_PLN,FUNC1);    //  0: motocon pcap0_0          1: usb0 usb0_ind0           2:  nc                      3: gpio4 gpio4_2
#else
	scu_pinmux(0x9,3,MD_PLN,FUNC0);		// 0: usb1_ind1
	scu_pinmux(0x9,4,MD_PLN,FUNC0); 	// 0: usb1_ind0
	scu_pinmux(0x2,5,MD_PLN,FUNC2); 	// 0: adctrig1					1: ucap0_2					2: usb1_vbus 		3: sgpio_14

	// make sure ULPI_CLK is not sollected on pinmux
	scu_pinmux(0x8,8,MD_PLN,FUNC0);    //  0:  nc                      1: usb1 ulpi_clk            2:  nc                      3:  nc
	scu_pinmux(0xC,0,MD_PLN,FUNC0);    //  0: sdio sdio_clk            1: usb1 ulpi_clk            2:  nc                      3: eth enet_rx_clk

	scu_pinmux(0x9,6,MD_PLN,FUNC0);    //  0: usb1 usb1_pwr_fault      1: motocon pwm0_4           2:  nc                      3:  nc

	// Set the correct mode of the USB1 pads.
	LPC_SCU->SFSPAIM = 0x2;
#endif
//	scu_pinmux(0x6,6,MD_PLN,FUNC0);    //  0: usb0 usb0_pwr_fault      1: ebi bls1                 2: gpio0 gpio0_5            3: sgpio sgpio_5
//	scu_pinmux(0x6,7,MD_PLN,FUNC2);    //  0: usb0 usb0_ind1           1: ebi a15                  2: gpio0 gpio0_6            3: sgpio sgpio_6
//	scu_pinmux(0x6,8,MD_PLN,FUNC2);    //  0: usb0 usb0_ind0           1: ebi a14                  2: gpio0 gpio0_7            3: sgpio sgpio_7
	...
	LPC_CGU->PL550M_0_C |= PD_ENABLE; 	// Power down PLL
						//	P			N
	LPC_CGU->PL550M_0_N = (98<<0) | (514<<12);
						//	SELP	SELI	SELR	MDEC
	LPC_CGU->PL550M_0_M &= ~(0xFFFF<0);
	LPC_CGU->PL550M_0_M |= /*(0xB<<17)|(0x10<<22)|(0<<28)|*/(0x7FFA<<0);
	LPC_CGU->PL550M_0_C =(SRC_XTAL<<24) | (0x3<<2)/*|(1<<1) */| CLKEN;

	LPC_SCU->SFSPCLK0 = 1 | (0x3<<2);	// Select clock-out

	// setup IDIVA_4 with PLL550 as source, divide ratio 4
	LPC_CGU->IDIVA_4_C = (0<<0) | ((4-1)<<2) | (1<<11) | (SRC_PL550M_0<<24);

	// setup IDIVE_256 with IDIV4 as source, divide ratio 25
	LPC_CGU->IDIVE_256_C = (0<<0) | ((25-1)<<2) | (1<<11) | (SRC_IDIV_0<<24);	// setup IDIV256

	// Select IDIV_4 as clock sorce for clock out
	// this now has a devide ratio of 1/100 with the PLL550 as source
	// On clock-out a 4.8MHz signal should be visible
	LPC_CGU->BASE_OUT_CLK = (0<<0) | (1<<11) | (SRC_IDIV_4<<24);

	// Set the USB clock source to PLL550 (480MHz)
...
#else
	SetClock(BASE_USB1_CLK, SRC_PL160M_0, DIV1);	// 60 MHz
#endif
...
//	LPC_USB->portsc1  =  (1<<14)| (0<16);
//	LPC_USB->portsc1  =  (1<<14)| (1<16);
//	LPC_USB->portsc1  =  (1<<14)| (2<16);
//	LPC_USB->portsc1  =  (1<<14)| (3<16);
//	LPC_USB->portsc1  =  (1<<14)| (4<16);
//	LPC_USB->portsc1  =  (1<<14)| (5<16);
//	LPC_USB->portsc1  =  (1<<14)| (6<16);
