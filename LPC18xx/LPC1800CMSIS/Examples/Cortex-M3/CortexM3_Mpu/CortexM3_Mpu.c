/**********************************************************************
* $Id: CortexM3_Mpu.c 28 2014-01-27 14:45:13Z danielru $		CortexM3_Mpu.c		2011-06-02
*//**
...
MPU_REGION_INIT LPC18xx_region[MPU_MAX_REGION] = {
...
int c_entry(void)
{
	...
	for(i=0;i< MPU_MAX_REGION;i++){
		...
		MPU->RBAR = LPC18xx_region[i].address; // update the base address for the region 0
		MPU->RASR = MPU_ATTR_AP(LPC18xx_region[i].ap) | MPU_ATTR_SHARE | MPU_ATTR_CACHE | MPU_ATTR_BUFFER
								| MPU_ATTR_SIZE(LPC18xx_region[i].size) | MPU_ATTR_EN;
	}
	...
	_DBG_("Press '2' to try to read memory from region 5\n\r"
		  "Read memory at this region is not allow, LED1 will blink...");
	while(_DG !='2');
