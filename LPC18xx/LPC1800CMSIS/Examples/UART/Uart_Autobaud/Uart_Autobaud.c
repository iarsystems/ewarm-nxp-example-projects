...
"UART1 - Auto Baud rate mode used \n\r";
...
void UART1_IRQHandler(void)
{
	...
	if (intsrc){
		...
		LPC_UART1->ACR |= ((intsrc & UART_IIR_ABEO_INT) ? UART_ACR_ABEOINT_CLR : 0) \
			...
			if (Synchronous == RESET)
			{
				...
				if (intsrc & UART_AUTOBAUD_INTSTAT_ABEO){
					...
					UART_IntConfig((LPC_USARTn_Type *)LPC_UART1, UART_INTCFG_ABEO, DISABLE);
					// Set Sync flag
					Synchronous = SET;
				}
...
int c_entry(void)
{
	...
	UART_IntConfig((LPC_USARTn_Type *)LPC_UART1, UART_INTCFG_ABEO, ENABLE);
	/* Enable UART Auto baudrate timeout interrupt */
	UART_IntConfig((LPC_USARTn_Type *)LPC_UART1, UART_INTCFG_ABTO, ENABLE);
...
/* ---------------------- Auto baud rate section ----------------------- */
    ...
    UART_ABCmd((LPC_USARTn_Type *)LPC_UART1, &ABConfig, ENABLE);
    print_menu();
    ...
    UART_Send((LPC_USARTn_Type *)LPC_UART1, syncmenu, sizeof(syncmenu), BLOCKING);
/* ---------------------- End of Auto baud rate section ----------------------- */
    ...
    while (exitflag == RESET)
    {
       len = 0;
        while (len == 0)
        {
            len = UART_Receive((LPC_USARTn_Type *)LPC_UART1, buffer, sizeof(buffer), NONE_BLOCKING);
        }
        ...
        while (idx < len)
        {
            if (buffer[idx] == 27)
            {
            	...
            	UART_Send((LPC_USARTn_Type *)LPC_UART1, menu3, sizeof(menu3), BLOCKING);
                exitflag = SET;
            }
    ...
    UART_DeInit((LPC_USARTn_Type *)LPC_UART1);
