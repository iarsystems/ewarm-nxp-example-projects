/**********************************************************************
* $Id: main.c 28 2014-01-27 14:45:13Z danielru $		main.c			2011-06-02
*//**
* @file		main.c
    ...
    while (pD->bLength) {
      if (pD->bDescriptorType == USB_INTERFACE_DESCRIPTOR_TYPE) {
		...
		case USB_DEVICE_CLASS_COMMUNICATIONS:
			ret = usb_cdc_init(hUsb, pIntfDesc, &usb_param.mem_base, &usb_param.mem_size);
			if (ret != LPC_OK)
				vCatchError(2); //"\r\n usb_hid_init error!!!"
			break;
		case USB_DEVICE_CLASS_CDC_DATA_INTERFACE:
			ret = usb_cdc_init(hUsb, pIntfDesc, &usb_param.mem_base, &usb_param.mem_size);
			if (ret != LPC_OK)
				vCatchError(2); //"\r\n usb_hid_init error!!!"
			break;
