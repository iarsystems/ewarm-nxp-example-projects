/**********************************************************************
* $Id: Uart_Autobaud.c 28 2014-01-27 14:45:13Z danielru $		Uart_Autobaud.c			2011-06-02
*//**
...
void UART0_IRQHandler(void)
{
	...
	if (intsrc){
		...
		LPC_USART0->ACR |= ((intsrc & UART_IIR_ABEO_INT) ? UART_ACR_ABEOINT_CLR : 0) \
			...
			if (Synchronous == RESET)
			{
				...
				if (intsrc & UART_AUTOBAUD_INTSTAT_ABEO){
					...
					UART_IntConfig((LPC_USARTn_Type *)LPC_USART0, UART_INTCFG_ABEO, DISABLE);
					// Set Sync flag
					Synchronous = SET;
				}
...
void print_menu(void)
{
	UART_Send((LPC_USARTn_Type *)LPC_USART0, menu1, sizeof(menu1), BLOCKING);
	UART_Send((LPC_USARTn_Type *)LPC_USART0, menu2, sizeof(menu2), BLOCKING);
}
...
/* ---------------------- Auto baud rate section ----------------------- */
    ...
    UART_ABCmd((LPC_USARTn_Type *)LPC_USART0, &ABConfig, ENABLE);
    print_menu();
    ...
    UART_Send((LPC_USARTn_Type *)LPC_USART0, syncmenu, sizeof(syncmenu), BLOCKING);
/* ---------------------- End of Auto baud rate section ----------------------- */
    ...
    while (exitflag == RESET)
    {
       len = 0;
        while (len == 0)
        {
            len = UART_Receive((LPC_USARTn_Type *)LPC_USART0, buffer, sizeof(buffer), NONE_BLOCKING);
        }
        ...
        while (idx < len)
        {
            if (buffer[idx] == 27)
            {
            	...
            	UART_Send((LPC_USARTn_Type *)LPC_USART0, menu3, sizeof(menu3), BLOCKING);
                exitflag = SET;
            }
            ...
            {
                /* Echo it back */
            	UART_Send((LPC_USARTn_Type *)LPC_USART0, &buffer[idx], 1, BLOCKING);
            }
            idx++;
        }
    }
    ...
    while (UART_CheckBusy((LPC_USARTn_Type *)LPC_USART0) == SET);

    // DeInitialize UART0 peripheral
    UART_DeInit((LPC_USARTn_Type *)LPC_USART0);
