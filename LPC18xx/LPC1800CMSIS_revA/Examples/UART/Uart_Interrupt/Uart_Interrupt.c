/**********************************************************************
* $Id: Uart_Interrupt.c 28 2014-01-27 14:45:13Z danielru $		Uart_Interrupt.c			2011-06-02
*//**
...
void UART_IntReceive(void)
{
	...
	while(1){
		...
		rLen = UART_Receive((LPC_USARTn_Type *)LPC_USART0, &tmpc, 1, NONE_BLOCKING);
		// If data received
		if (rLen){
...
void UART_IntTransmit(void)
{
    ...
    UART_IntConfig((LPC_USARTn_Type *)LPC_USART0, UART_INTCFG_THRE, DISABLE);
	...
	while (!__BUF_IS_EMPTY(rb.tx_head,rb.tx_tail))
    {
    	...
    	if (UART_Send((LPC_USARTn_Type *)LPC_USART0, (uint8_t *)&rb.tx[rb.tx_tail], 1, NONE_BLOCKING)){
	...
	if (__BUF_IS_EMPTY(rb.tx_head, rb.tx_tail)) {
		...
		TxIntStat = SET;
    	UART_IntConfig((LPC_USARTn_Type *)LPC_USART0, UART_INTCFG_THRE, ENABLE);
    }
}
...
int c_entry(void)
{
    ...
    while (exitflag == RESET)
    {
       len = 0;
        while (len == 0)
        {
            len = UARTReceive((LPC_USARTn_Type *)LPC_USART0, buffer, sizeof(buffer));
        }
        ...
        while (idx < len)
        {
            if (buffer[idx] == 27)
            {
            	...
            	UARTSend((LPC_USARTn_Type *)LPC_USART0, menu3, sizeof(menu3));
                exitflag = SET;
            }
            ...
            {
                /* Echo it back */
            	UARTSend((LPC_USARTn_Type *)LPC_USART0, &buffer[idx], 1);
            }
            idx++;
        }
    }
    ...
    while (UART_CheckBusy((LPC_USARTn_Type *)LPC_USART0));
