...
unsigned int		isOutBufferProcessing = 0;
 ...
 *---------------------------------------------------------------------------*/
/* Buffer masks */
#define CDC_BUF_SIZE               (2*(USB_CDC_BUFSIZE))               // Output buffer in bytes (power 2)
...
void CDC_NotificationIn (void) {
  ...
  NotificationBuf[9] = (CDC_SerialState >>  8) & 0xFF;
	while(!Endpoint_IsINReady(CDC_CEP_IN));
  USB_WriteEP (CDC_CEP_IN, &NotificationBuf[0], 10);   // send notification
}
void CDC_BulkOutNak(void){
	if(CDC_BUF_EMPTY(CDC_OutBuf) && !isOutBufferProcessing){
    USB_ReadReqEP(CDC_DEP_OUT, &BulkBufOut[0], 64);
		isOutBufferProcessing = 1;
	}
