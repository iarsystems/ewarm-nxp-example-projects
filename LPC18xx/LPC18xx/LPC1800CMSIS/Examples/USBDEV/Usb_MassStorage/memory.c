/**********************************************************************
* $Id: memory.c 28 2014-01-27 14:45:13Z danielru $		memory.c				2011-06-02
*//**
* @file		memory.c
...
/* Define whether the part is Eagle without flash, or Raven with flash */
//#ifdef FLASHLESS
...
void SysTick_Handler (void) 					// SysTick Interrupt Handler @ 500Hz
{
	...
	if ( cnt < systicks )	   					// 0.5s
	{
		...
		if(LPC_GPIO4->PIN&(1<<14))
		  LPC_GPIO4->CLR = (1<<14);				// Toggle led
		else
		  LPC_GPIO4->SET = (1<<14);
	}
}
