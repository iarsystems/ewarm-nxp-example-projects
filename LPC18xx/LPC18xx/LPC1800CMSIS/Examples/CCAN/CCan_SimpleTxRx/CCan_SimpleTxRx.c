/**********************************************************************
* $Id: CCan_SimpleTxRx.c 28 2014-01-27 14:45:13Z danielru $		CCan_SimpleTxRx.c		2011-06-02
*//**
...
int c_entry(void) { /* Main Program */
	...
	/* Setup IO pin */
	scu_pinmux(0x3,2,MD_PUP,FUNC2);		/* PE.3 CAN TD1, FUNC1 */
	scu_pinmux(0x3,1,MD_PUP,FUNC2);		/* PE.2	CAN RD1, FUNC1 */
	...
	/* Init CAN */
	/** Should set CAN Peripheral Clock = 72MHz
		we have 72MHz/9 = 8Mhz */
	CAN_Init( CAN_BITRATE500K8MHZ, CLKDIV9 , TX_callback, RX_callback);

	lpc_printf("CAN init\n\r");

	#if(BASIC_MODE | SILENT_MODE | LOOPBACK_MODE)
		LPC_C_CAN->CNTL |= CTRL_TEST;				// Enable test mode
		#if BASIC_MODE
	 		LPC_C_CAN->TEST |= TEST_BASIC;			// Enable basic mode
		#endif
		#if SILENT_MODE
	 		LPC_C_CAN->TEST |= TEST_SILENT;			// Enable silent mode
		#endif
		#if LOOPBACK_MODE
	  		LPC_C_CAN->TEST |= TEST_LBACK;			// Enable Loopback mode
		#endif
	#endif
