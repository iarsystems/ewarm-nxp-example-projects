...
#include "tcpip.h"
#include "EMAC.h"         // Line added
#include <string.h>       // Line added
    ...
    case LISTENING :
    {
      if (TCPFlags & TCP_ACTIVE_OPEN)            // stack has to open a connection?
        if (TCPFlags & IP_ADDR_RESOLVED)         // IP resolved?
          ...
          {
//            TCPSeqNr = ((unsigned long)ISNGenHigh << 16) | (T0TC & 0xFFFF);  // changed from TAR to T0TC; 
...
{
  unsigned long Sum = 0;
  unsigned short * piStart;                        // Pointer added to correct expression
  ...
  piStart = Start;                               // Line added
  while (Count > 1) {                            // sum words
//  Sum += *((unsigned short *)Start)++;		     // Line replaced with following line
    Sum += *piStart++;
    Count -= 2;
  }

  if (Count)                                     // add left-over byte, if any
//  Sum += *(unsigned char *)Start; 	         // Line replaced with following line
