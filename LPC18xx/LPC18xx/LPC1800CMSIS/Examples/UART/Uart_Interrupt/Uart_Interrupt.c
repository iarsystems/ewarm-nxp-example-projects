...
void UART_IntTransmit(void)
{
	...
	while (!__BUF_IS_EMPTY(rb.tx_head,rb.tx_tail))
    {
    	...
    	if (UART_Send((LPC_USARTn_Type *)LPC_UART1, (uint8_t *)&rb.tx[rb.tx_tail], 1, NONE_BLOCKING)){
...
void print_menu(void)
{
	...
	while(tmp) {
		tmp2 = UARTSend((LPC_USARTn_Type *)LPC_UART1, pDat, tmp);
		pDat += tmp2;
		tmp -= tmp2;
	}
...
int c_entry(void)
{
    ...
    while (exitflag == RESET)
    {
       len = 0;
        while (len == 0)
        {
            len = UARTReceive((LPC_USARTn_Type *)LPC_UART1, buffer, sizeof(buffer));
        }
        ...
        while (idx < len)
        {
            if (buffer[idx] == 27)
            {
            	...
            	UARTSend((LPC_USARTn_Type *)LPC_UART1, menu3, sizeof(menu3));
                exitflag = SET;
            }
            ...
            {
                /* Echo it back */
            	UARTSend((LPC_USARTn_Type *)LPC_UART1, &buffer[idx], 1);
            }
            idx++;
        }
    }
    ...
    while (UART_CheckBusy((LPC_USARTn_Type *)LPC_UART1));
