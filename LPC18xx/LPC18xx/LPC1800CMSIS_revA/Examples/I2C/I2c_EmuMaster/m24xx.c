/**********************************************************************
* $Id: m24xx.c 28 2014-01-27 14:45:13Z danielru $		m24xx.c	 2012-05-15
*//**
* @file		m24xx.c
...
extern unsigned char lcd_configdata[128];

/*************************************************************************************
 *  Read the configuration data from the EEPROM on the display module.
 *
 *
 *
 *************************************************************************************/
void m24xx_read(void)
{
	//m24xx_eeprom_read (0, 8, value[]);
}
            ...
            if (i2cemu_send_byte ((I2C_ADDRESS_M24C64 << 1) | 1))   /* Address byte */
            {
                for (i = 0; i < length; i++)
                {
                    ...
                    value[i] = i2cemu_receive_byte(i < length-1);
									  //value[i] = i;
									  //lcd_configdata[i] = i2cemu_receive_byte(i < (length-1));
                }
            }
            result = true;
          }
