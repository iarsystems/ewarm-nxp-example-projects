...
#endif
#ifdef USE_USB0
	CoreM3Freq = CGU_GetPCLKFrequency(CGU_PERIPHERAL_M3CORE);
	/* Disable PLL first */
...
#else
	/* remember to set M3 Core Clock to 60Mhz */
...
/*
*  Check if EP IN is ready or not
*    Parameters:      EPNum: Endpoint Number
*                       EPNum.0..3: Address
*                       EPNum.7:    Dir
*    Return Value:    1 if EP is ready, otherwise return 0
*/
uint32_t Endpoint_IsINReady(uint32_t EPNum)
{
	DTD_T*  pDTD ;
	pDTD = (DTD_T*)&ep_TD[ EPAdr(EPNum) ];
		if((pDTD->next_dTD & 1)&& ((pDTD->total_bytes & 1<<7)==0))
			return 1;
		return 0;
}
/*
