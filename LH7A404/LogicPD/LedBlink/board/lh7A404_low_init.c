...
{
  // nCS0 - Flash
  SMC_BCR6_bit.RBLE = 0;  // all driven HIGH during memory Reads
  SMC_BCR6_bit.WP   = 1;  // Write protection enable
  SMC_BCR6_bit.PME  = 1;  // Page Mode Enable
  // Read/Write Access Delay
  SMC_BCR6_bit.WST1 = EMC_DIV(FLASH_WAIT_RD,HCLK_FREQ);
  // Asynchronous Page Mode Delay
  SMC_BCR6_bit.WST2 = EMC_DIV(FLASH_WAIT_PAGE,HCLK_FREQ);
  ...
  SMC_BCR7_bit.RBLE = 0;  // all driven HIGH during memory Reads
...
//  SDRAMC_GBLCNFG_bit.Initialize = 1;
  SDRAMC_GBLCNFG = 1;
  ...
  Dummy = *(unsigned long *)(DMEM_CS0_BASE_ADDR + (0x22UL << 10));
