...
10 Jun 05
Enabling FIFO buffers with U0FCR = 1; per Anders Lundgren's advice.

20 Oct 08
Adapt for Micro-Blox LPC2148 board. Change from UART1 to UART0
Please set jumplers:
...
int main(void)
{
  ...
  /*Configure the pins that are connected to RX and TX on UART0 */
  PINSEL0 = (1<<ENABLE_UART0_RX) | (1<< ENABLE_UART0_TX); // PINSEL0 = 0x00005

  /*Configure UART0 to 9600 buad, 8 bit, 1 stop, no parity */
  PCONP_bit.PCURT0 = 1;
  U0FCR = 1;                      // Enable FIFOs whether used or not
  SetBit(U0LCR,DIVISOR_LATCH_SET_bit); // U0LCR = 0X80-enable access to divisor
                                  // latch bit, necessary for setting baud rate
  U0LCR |= EIGHT_BIT_CHARS;       // Eight bits U0LCR = 0X83
...
{
    while (!(U0LSR &0x20)); //U0THR THRE bit set when U0THR contains valid data
    U0THR = byte;
}
