...
int main()
{
  ...
  /* Read potentiometer - will stay here until SW6 is pushed */
  read_potentiometer();
   ...
   * 3.		LPSTOPx mode
   *
   * 	--> SW9 push button (PTB27) can be used as wake-up
   *
   ***********************************************************************/
  /* Setup Wakeup Unit for SW9 (PTB27) wake up out of LPSTOPx modes */
  enable_wkpu(WKUP_SOURCE_14, RISING_EDGE_ENABLED);	// button push vss pin

  printf("----> Entering LPSTOP3 mode...[ SW9 = Exit LPSTOPx ]\n");
...
void read_potentiometer()
{
  ...
  ADC0->HC[0] = 0x00000008;    /* channel 8 */
  ADC0->PCTL = 0x00000100;   /* disable I/O functionality on channel 8. Use as ADC input only */
  ...
  do
  {
    ...
    if (i % 9000 == 0)
    {
    	printf("\r\tCurrent Potentiometer Reading:  %4d mV....[ Push SW6 to continue ]", voltage);
    	LEDS_TOGGLE;
    }
...
{
	/* Enable buttons on Faraday EVB*/
	PUSH_BUTTONS_INIT;

	/* Setup UART0 for clocking on all modes */
	CCM->CCGR0 |= CCM_CCGR0_CG7(CLK_ON_ALL_MODES);

	/* Setup PTB27 (PORT3[1] RGPIO97) for interrupt on either edge for SW9 push */
	PORT3->PCR[1] |= PORT_PCR_IRQC(0x0);		//Disable because we don't want port pin interrupt, we want wakeup unit interrupt
