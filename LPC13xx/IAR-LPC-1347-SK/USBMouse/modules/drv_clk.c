 ...
 *
 *    File name   : drv_clk.c
 *    Description : NXP LPC1347 Clock Drv
 *
 *    History :
 *    1. Date        : January 2012
...
/*************************************************************************
 * Function Name: CLK_SetSSP1Clk
 * Parameters: uint32_t div
 *             
 * Return: void
 *
 * Description: set SSP1 clock divider
 *
 *
 *************************************************************************/
void CLK_SetSSP1Clk(uint32_t div)
{
  /* SSP1 clock divider set */
  SSP1CLKDIV_bit.DIV = div & 0xFF;
}
...
void CLK_SetUsbPll(clk_enable_t pllenable, uint32_t pllm, uint32_t pllp)
{
  ...
  if(CLK_ENABLE == pllenable)
  {
    /* Select Main oscillator as USB PLL in clock */
...
uint32_t freq = 0;
  ...
  case CLK_SSP1:
    /* SSP1 divider is on */
    if(SSP1CLKDIV_bit.DIV)
    {
      freq = CLK_GetClock(CLK_MAIN)/SSP1CLKDIV_bit.DIV;
    }
    break;
