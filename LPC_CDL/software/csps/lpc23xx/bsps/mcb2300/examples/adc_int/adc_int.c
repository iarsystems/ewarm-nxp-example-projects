/***********************************************************************
 * ID: adc_int.c 2008-11-09		HieuNguyen		$
 *
 * Project: ADC driver example
 *
 * Description:
 *     Read ADC0.0 value in interrupt mode and display via UART0 
...
" Use ADC with 10-bit resolution at freq = 1MHz, read in interrupt mode\n\r"
...
// Call back function of ADC device when using in interrupt (ASYNC mode)
void usr_adc_cbs (void)
{
	// Set complete flag
	complete = TRUE;
}
...
{	
	...
	if ((VIC_InstallIRQ (ADC0_INT, (void *) ADC_ISR, 0x01)) == _ERROR)
	{
		return 0;
	}
	...
	// Enable interrupt for ADC0 channel 0
	ADC_EnableInt (adcdev, 0);
	...
	while (1)
	{
		...
		// Wait for complete flag set by call back function
		while (complete == FALSE);
		
		// Read ADC value from channel 0
		ADCValue = ADC_ReadAsync (adcdev, 0);
