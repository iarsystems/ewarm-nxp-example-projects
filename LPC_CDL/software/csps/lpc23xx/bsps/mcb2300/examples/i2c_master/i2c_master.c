/***********************************************************************
 * i2c_master.c 2008-10-29 --- HieuNguyen
 *
 * Project: I2C driver example
 *
 * Description:
 * 		This code is set I2C in MASTER mode
...
" To communicate with the other that uses I2C function as SLAVE mode \n\r"
" Write and read 64 bytes to/from I2C slave \n\r"
"********************************************************************************\n\r";
...
{		
	...
	if ((i2cdev = I2C_Init (I2C_PORT_1, I2C_MASTER_MODE, 0x00, 100000)) == 0x00)
	{
		return 0;
	}
	...
	UART_Puts_ (uartdev, "Press 1 to send data!");
	
	while (UART0Read() != '1');
	
	// Init and print data content before sending
	DataInit();
	
	UART_Puts_ (uartdev, "Sending data...");
	
	/* Send some data in polling mode first */
	// Write 64 bytes
	monitor = I2C_WritePoll (i2cdev, SLVADDR, wdata, sizeof (wdata));
	UART_Puts_ (uartdev, "Data sending complete!");
	UART_Puts_ (uartdev, "Press 2 to receive data!");
	
	while (UART0Read() != '2');
	
	UART_Puts_ (uartdev, "Receiving data...");
	//Read back 64 bytes of data from slave in ASYNC mode 
	I2CFlag = 0;
	monitor = I2C_ReadAsync (i2cdev, SLVADDR, rdata, sizeof (rdata));
	/* Wait for I2CFlag == 1 from user callback function */
	while (I2CFlag == 0);
	
	UART_Puts_ (uartdev, "Data received complete!");
