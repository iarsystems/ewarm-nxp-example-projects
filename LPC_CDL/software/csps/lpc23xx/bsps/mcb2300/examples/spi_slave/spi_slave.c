/***********************************************************************
 * $Id: spi_slave.c 28 2014-01-27 14:45:13Z danielru $
 *
 * Project: LPC23xx driver
 *
 * Description:	
 * 		This example will set SPI in SLAVE mode
...
" Use SPI function in SLAVE mode \n\r"
" To communicate with the other that uses SPI function as MASTER mode \n\r"
" Transfer 100 bytes of data with master \n\r"
"********************************************************************************\n\r";
...
int c_entry (void)
{
	...
	if ((spidev = SPI_Init (0, SPI_SLAVE, 100000, SPI_DATABIT_8, SPI_FIRST_CLK_EDGE,
...
//	SPI_ReadWritePoll (spidev, &rdata, &wdata, sizeof(wdata));
	SPI_ReadWriteAsync (spidev, &rdata, &wdata, sizeof(wdata));
	while (SPIFlag == FALSE);
