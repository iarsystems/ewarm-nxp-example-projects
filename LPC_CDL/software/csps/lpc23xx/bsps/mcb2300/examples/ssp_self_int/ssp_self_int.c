 ...
 * MOSI and MISO pin on SSP pinout must be cross-connected together
 * Transfer 64 bytes of data (in interrupt mode) and display result
 ***********************************************************************
...
int c_entry (void)
{
	...
	// Install handler for SSP interrupt
	if ((VIC_InstallIRQ (SPI0_INT, (void *) SSP0_ISR, 0x01)) == _ERROR)
	{
		return 0;
	}
	
	// Set callback function
	SSP_SetCBS (sspdev, (void *)user_callback);
	...
	SSP_ReadWriteAsync (sspdev, readdata, writedata, sizeof (writedata));
