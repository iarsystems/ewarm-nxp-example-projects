/***********************************************************************
 * $Id: sdcard_no_dma_example.c 28 2014-01-27 14:45:13Z danielru $
 *
...
typedef struct _tBPB1216_
{
  UNS_8 BS_DrvNum;
  UNS_8 BS_BootSig;
  UNS_32 BS_VolID;
  UNS_8 BS_VolLab[11];
  UNS_8 BS_FilSysType[8];
}tBPB1216;

typedef struct _tBPB32_
{
  UNS_32 BPB_FATSz32;
  UNS_16 BPB_ExtFlags;
  UNS_16 BPB_FSVer;
...
typedef struct _tBPB_
{
  UNS_8 BS_jmpBoot[3];
  UNS_8 BS_OEMName[8];
  UNS_16 BPB_BytsPerSec;
  UNS_8 BPB_SecPerClus;
  UNS_16 BPB_RsvdSecCnt;
  UNS_8 BPB_NumFATs;
  UNS_16 BPB_RootEntCnt;
  UNS_16 BPB_TotSec16;
  UNS_8 BPB_Media;
  UNS_16 BPB_FATSz16;
  UNS_16 BPB_SecPerTrk;
  UNS_16 BPB_NumHeads;
  UNS_32 BPB_HiddSec;
  UNS_32 BPB_TotSec32;
  union
  {
    tBPB1216 BPB1216;
    tBPB32 BPB32;
  }typeFields;      /* fields that are different between
           fat12/16 and fat32 */
}tBPB;
 ...
 *
 **********************************************************************/
INT_32 sdmmc_read_block(UNS_32 *buf,
 ...
 *
 **********************************************************************/
INT_32 sdmmc_write_block(UNS_32 *buf,
                         INT_32 numblks, /* Must be 1 */
                         UNS_32 index,
                         SD_CMDRESP_T *resp)
{
  ...
  if (sdcardtype == 1)
  {
    ...
    while ((validocr == 0) && (ocrtries >= 0))
    {
      ...
      if ((resp.cmd_resp [1] & SDMMC_OCR_MASK) == 0)
      {
        ...
        // EA. Without this delay we end up in the prefetch handler??
...
static BOOL_32 mmc_blkdev_init(void)
{
  return sdmmc_setup();
}

static BOOL_32 mmc_blkdev_read(void *mmcbuff, UNS_32 sector, UNS_32 sec_cnt)
{
  INT_32 result;
  SD_CMDRESP_T resp;

  result = sdmmc_read_block(mmcbuff, 1, sector*512, &resp);

  return result == _NO_ERROR;
}

static BOOL_32 mmc_blkdev_deinit(void)
{
  uart_output("MMC: blkdev_deinit\r\n");
  return TRUE;
}

static BOOL_32 test_sdcard(void)
{
  int i = 0;
  UNS_8 buff[110];
  ...
  if (fat_init(mmc_blkdev_init, mmc_blkdev_read, mmc_blkdev_deinit) == FALSE)
  {
    uart_output("fat_init failed\r\n");
    return FALSE;
  }

  if (fat_file_read_nocache("testfile.txt", (UNS_8*)buff, 100) != 100)
  {
    uart_output("Failed to read file 'testfile.txt'\r\n");
    return FALSE;
  }

  uart_output("'testfile.txt opened\r\n");

  for ( i = 0; i < 100; i ++)
  {
    if (buff[i] != ('0' + (i%10)))
    {
      uart_output("expected data not found in 'testfile.txt'\r\n");
      return FALSE;
    }
  }


  uart_output("Test OK: Expected data Found in 'testfile.txt'\r\n");
  return TRUE;
}
