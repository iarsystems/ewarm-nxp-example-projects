/***********************************************************************
 * $Id: phy3250_nand.c 28 2014-01-27 14:45:13Z danielru $
 *
 ...
 *     readbuff : Pointer to write buffer >= 512 bytes
 *     extrabuff: Extra data to write to spare area, 6 bytes
 *
 * Outputs: None
 *
 * Returns: Returns 512, or 0 if a write error occurs
 *
 * Notes: None
 *
 **********************************************************************/
                         ...
                         BOOL_32 skipbad)
{
  ...
  while (num_sectors > 0)
  {
    ...
    while (checkblk == TRUE)
    {
      mlc_read_page(block, 0, extrabuff, &extrabuff [512]);
      if (extrabuff [NAND_LPCBADBLOCK_OFFS] !=
...
void phy3250_nand_wp(BOOL_32 enable)
{
  ...
  {
    gpio_set_gpo_state(0, P3_STATE_GPO(19));
  }
}
