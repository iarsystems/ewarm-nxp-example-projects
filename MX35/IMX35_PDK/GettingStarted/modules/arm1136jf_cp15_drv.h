...
#define L1_SECTIONS_ENTRY(Numb, VirtAddr ,PhAddr,TEX, AP, Domain,C,B) \
  { Numb, VirtAddr, PhAddr, ((TEX<<12) | (AP << 10) | (Domain << 5) | (C << 3) | (B << 2) | TtL1Section)}
#define L1_SUPERSECTIONS_ENTRY(Numb, VirtAddr ,PhAddr,TEX,AP, Domain,C,B) \
  { Numb*16, VirtAddr, PhAddr, ((TEX<<12) |(AP << 10) | (Domain << 5) | (C << 3) | (B << 2) | TtL1SuperSection)}

#define L2_LARGE_PAGE_ENTRY(Numb, VirtAddr ,PhAddr, Ap0, Ap1, Ap2, Ap3,C,B) \
  { Numb*16, VirtAddr, PhAddr, \
  ((Ap3 << 10) | (Ap2 << 8) | (Ap1 << 6) | (Ap0 << 4) | (C << 3) | (B << 2) |  \
    TtL2LargePage)}
#define L2_SMALL_PAGE_ENTRY(Numb, VirtAddr ,PhAddr, Ap0, Ap1, Ap2, Ap3,C,B) \
  { Numb, VirtAddr, PhAddr, \
  ((Ap3 << 10) | (Ap2 << 8) | (Ap1 << 6) | (Ap0 << 4) | (C << 3) | (B << 2) |  \
    TtL2SmallPage)}

#define L2_EXTSMALL_PAGE_ENTRY(Numb, VirtAddr ,PhAddr, TEX,Ap,C,B) \
  { Numb, VirtAddr, PhAddr, \
  ((TEX<<6) | (Ap << 4) | (C << 3) | (B << 2) |  \
  TtL2ExtSmallPage)}
...
typedef enum
{
  TtL1Invalid = 0, TtL1CoarsePage, TtL1Section, TtL1SuperSection = 0x40002,
} TtL1EntryType_t;

typedef enum
{
  TtL2Invalid = 0, TtL2LargePage, TtL2SmallPage, TtL2ExtSmallPage,
} TtL2EntryType_t;

typedef enum
{
  TableInvalid = 0, TableL1, TableL2_Coarse,
} TableType_t;
