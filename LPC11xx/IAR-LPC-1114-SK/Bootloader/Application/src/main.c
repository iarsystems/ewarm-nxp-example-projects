...
int main(void)
{
	...
	/* Initialize the GPIO (P3.0) that will be used to control the LED */
	LPC_GPIO3->DIR |= (1UL << 0);

	/* Initialize the GPIO (P0.7) that will be used to initiate an application
	...
	while(1)
	{
		/* Check if P0.7 is being held low, if so user is requesting an
		   application download is initiated using the secondary bootloader */
		if ((LPC_GPIO0->DATA & (1UL << 7)) == 0)
		{
...
void SysTick_Handler(void)
{
	if (u8LedOn)
	{
		LPC_GPIO3->DATA |= (1UL << 0); /* Turn LED on */
	}
	else
	{
		LPC_GPIO3->DATA &= ~(1UL << 0); /* Turn LED off */
	}
	u8LedOn = !u8LedOn;
}
