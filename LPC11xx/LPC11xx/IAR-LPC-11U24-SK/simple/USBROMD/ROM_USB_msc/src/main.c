...
extern ErrorCode_t usb_msc_init(USBD_HANDLE_T hUsb, USB_INTERFACE_DESCRIPTOR* pIntfDesc, uint32_t* mem_base, uint32_t* mem_size);
...
/* Initialize Descriptor pointers */
  ...
  if (ret == LPC_OK)
  {
    ...
    if (ret == LPC_OK)
    {
      ...
      UARTSend((uint8_t *)"\r\n MSC initialization done !!!", 32);
    }
    else
    {
      UARTSend((uint8_t *)"\r\n MSC initialization error!!!", 32);
    }
  }
  else
  {
