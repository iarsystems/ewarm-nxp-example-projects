...
Int32U intr;
  ...
  {
    INTSTAT = bmUSB_DevStatusInterrupt;
    Int32U dev_stat = DEVCMDSTAT;
    dev_stat &= ~DEVCMDSTAT_WR_1_CLR_MASK;
    if(DEVCMDSTAT_bit.DCON_C)
    {
      DEVCMDSTAT = dev_stat | bmUSB_ConnectChange;
      UsbDevConnectCallback(DEVCMDSTAT_bit.DCON);
    }
    if(DEVCMDSTAT_bit.DSUS_C)
    {
      DEVCMDSTAT = dev_stat | bmUSB_SuspendChange;
      UsbDevSuspendCallback(DEVCMDSTAT_bit.DSUS);
    }
    if(DEVCMDSTAT_bit.DRES_C)
    {
      DEVCMDSTAT = dev_stat | bmUSB_BusReset;
      USB_HwReset();
      UsbDevResetCallback();
    }
  }
  ...
  while(intr)
  {
    if(intr & mask)
    {
         ...
         )
      {
        ...
        Int32U dev_stat = DEVCMDSTAT;
        dev_stat &= ~DEVCMDSTAT_WR_1_CLR_MASK;
        DEVCMDSTAT = dev_stat | bmUSB_EpSetupPacket;
