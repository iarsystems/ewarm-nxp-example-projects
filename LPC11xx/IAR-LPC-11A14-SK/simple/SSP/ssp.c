/****************************************************************************
 *   $Id: ssp.c 28 2014-01-27 14:45:13Z danielru $
...
#if 0
    LPC_IOCON->TDI_PIO0_6 &= ~0x07; /*  SSP I/O config */
    LPC_IOCON->TDI_PIO0_6 |= 0x04;  /* SSP MISO */
    LPC_IOCON->PIO0_4 &= ~0x07;
    LPC_IOCON->PIO0_4 |= 0x04;  /* SSP MOSI */
    LPC_IOCON->SWCLK_PIO0_5 &= ~0x07;
    LPC_IOCON->SWCLK_PIO0_5 |= 0x04;  /* SSP CLK */
#if USE_CS
    LPC_IOCON->PIO0_1 &= ~0x07;
    LPC_IOCON->PIO0_1 |= 0x04;    /* SSP SSEL */
#else
    /* Enable AHB clock to the GPIO domain. */
    LPC_SYSCON->SYSAHBCLKCTRL |= (1<<6);

    LPC_IOCON->PIO0_1 &= ~0x07;   /* SSP SSEL is a GPIO pin */
    /* port0, bit 2 is set to GPIO output and high */
    GPIOSetDir( PORT0, 1, 1 );
    GPIOSetBitValue( PORT0, 1, 1 );
#endif
#endif

#if 1
    LPC_IOCON->PIO0_22 &= ~0x07;  /*  SSP I/O config */
    LPC_IOCON->PIO0_22 |= 0x01;   /* SSP MISO */
    LPC_IOCON->PIO0_19 &= ~0x07;
    LPC_IOCON->PIO0_19 |= 0x03;   /* SSP MOSI */
    LPC_IOCON->PIO0_20 &= ~0x07;
    LPC_IOCON->PIO0_20 |= 0x02;   /* SSP CLK */
#if USE_CS
    LPC_IOCON->PIO0_18 &= ~0x07;
    LPC_IOCON->PIO0_18 |= 0x02;   /* SSP SSEL */
#else
    /* Enable AHB clock to the GPIO domain. */
    LPC_SYSCON->SYSAHBCLKCTRL |= (1<<6);

    LPC_IOCON->PIO0_18 &= ~0x07;    /* SSP SSEL is a GPIO pin */
    /* port0, bit 2 is set to GPIO output and high */
    GPIOSetDir( PORT0, 18, 1 );
    GPIOSetBitValue( PORT0, 18, 1 );
#endif
#endif

#if 0
    LPC_IOCON->PIO1_2 &= ~0x07;   /*  SSP I/O config */
    LPC_IOCON->PIO1_2 |= 0x02;    /* SSP MISO */
#if 1
    LPC_IOCON->PIO1_3 &= ~0x07;
    LPC_IOCON->PIO1_3 |= 0x02;    /* SSP MOSI */
#else
    LPC_IOCON->PIO1_7 &= ~0x07;
    LPC_IOCON->PIO1_7 |= 0x02;    /* SSP MOSI */
#endif
    LPC_IOCON->PIO1_0 &= ~0x07;
    LPC_IOCON->PIO1_0 |= 0x02;    /* SSP CLK */
#if USE_CS
    LPC_IOCON->PIO1_1 &= ~0x07;
    LPC_IOCON->PIO1_1 |= 0x02;    /* SSP SSEL */
#else
    /* Enable AHB clock to the GPIO domain. */
    LPC_SYSCON->SYSAHBCLKCTRL |= (1<<6);

    LPC_IOCON->PIO1_1 &= ~0x07;   /* SSP SSEL is a GPIO pin */
    /* port0, bit 2 is set to GPIO output and high */
    GPIOSetDir( PORT1, 1, 1 );
    GPIOSetBitValue( PORT1, 1, 1 );
#endif
#endif
  }
  else    /* port number 1 */
...
#if 1
    LPC_IOCON->PIO0_14 &= ~0x07;  /*  SSP I/O config */
    LPC_IOCON->PIO0_14 |= 0x01;   /* SSP MISO */
    LPC_IOCON->PIO0_27 &= ~0x07;
    LPC_IOCON->PIO0_27 |= 0x01;   /* SSP MOSI */
    LPC_IOCON->TDO_PIO0_8 &= ~0x07;
    LPC_IOCON->TDO_PIO0_8 |= 0x04;  /* SSP CLK */
#if USE_CS
    LPC_IOCON->PIO0_25 &= ~0x07;
    LPC_IOCON->PIO0_25 |= 0x02;   /* SSP SSEL */
#else
    /* Enable AHB clock to the GPIO domain. */
    LPC_SYSCON->SYSAHBCLKCTRL |= (1<<6);

    LPC_IOCON->PIO0_25 &= ~0x07;    /* SSP SSEL is a GPIO pin */
    /* port0, bit 2 is set to GPIO output and high */
    GPIOSetDir( PORT0, 25, 1 );
    GPIOSetBitValue( PORT0, 25, 1 );
#endif
#endif

#if 0
    LPC_IOCON->PIO0_26 &= ~0x07;  /*  SSP I/O config */
    LPC_IOCON->PIO0_26 |= 0x02;   /* SSP MISO */
    LPC_IOCON->PIO0_31 &= ~0x07;
    LPC_IOCON->PIO0_31 |= 0x02;   /* SSP MOSI */
    LPC_IOCON->PIO1_5 &= ~0x07;
    LPC_IOCON->PIO1_5 |= 0x02;    /* SSP CLK */
#if USE_CS
    LPC_IOCON->PIO1_4  &= ~0x07;
    LPC_IOCON->PIO1_4  |= 0x02;   /* SSP SSEL */
#else
    /* Enable AHB clock to the GPIO domain. */
    LPC_SYSCON->SYSAHBCLKCTRL |= (1<<6);

    LPC_IOCON->PIO1_4 &= ~0x07;   /* SSP SSEL is a GPIO pin */
    /* port0, bit 2 is set to GPIO output and high */
    GPIOSetDir( PORT1, 4, 1 );
    GPIOSetBitValue( PORT1, 4, 1 );
#endif
#endif

#if 0
    LPC_IOCON->PIO1_8 &= ~0x07;   /*  SSP I/O config */
    LPC_IOCON->PIO1_8 |= 0x02;    /* SSP MISO */
#if 1
    LPC_IOCON->PIO0_30 &= ~0x07;
    LPC_IOCON->PIO0_30 |= 0x02;   /* SSP MOSI */
#else
    LPC_IOCON->PIO1_6 &= ~0x07;
    LPC_IOCON->PIO1_6 |= 0x02;    /* SSP MOSI */
#endif
    LPC_IOCON->PIO0_29 &= ~0x07;
    LPC_IOCON->PIO0_29 |= 0x02;   /* SSP CLK */
#if USE_CS
    LPC_IOCON->PIO0_28 &= ~0x07;
    LPC_IOCON->PIO0_28 |= 0x02;   /* SSP SSEL */
#else
    /* Enable AHB clock to the GPIO domain. */
    LPC_SYSCON->SYSAHBCLKCTRL |= (1<<6);

    LPC_IOCON->PIO0_28 &= ~0x07;    /* SSP SSEL is a GPIO pin */
    /* set to GPIO output and high */
    GPIOSetDir( PORT0, 28, 1 );
    GPIOSetBitValue( PORT0, 28, 1 );
#endif
#endif
  }
  return;
