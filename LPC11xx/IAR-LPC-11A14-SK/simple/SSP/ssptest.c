/****************************************************************************
 *   $Id: ssptest.c 28 2014-01-27 14:45:13Z danielru $
...
/*****************************************************************************
** Function name:   ATM25_SFlashTest
**
** Descriptions:    Serial Flash(Atmel 25xxx) test
**
** parameters:      port #
** Returned value:    None
**
*****************************************************************************/
void ATM25_SFlashTest( uint8_t portNum )
{
  uint32_t i, timeout;
  ...
  if ( portNum == 0 )
  {
    ...
    for ( i = 0; i < 0x80; i++ ); /* delay minimum 250ns */
    GPIOSetBitValue( PORT0, 2, 0 );
    src_addr[0] = ATM_CHIP_ERASE; /* Chip erase on Atmel 25xxx */
    SSP_Send( portNum, (uint8_t *)src_addr, 1 );
    GPIOSetBitValue( PORT0, 2, 1 );
...
/*****************************************************************************
** Function name:   SST26_SFlashTest
**
** Descriptions:    Serial Flash(SST 26xxx) test
**
** parameters:      port #
** Returned value:    None
**
*****************************************************************************/
void SST26_SFlashTest( uint8_t portNum )
{
  uint32_t i, timeout;
  ...
  GPIOSetBitValue( PORT0, 2, 0 );
  /* Test SST26xxx Serial flash. */
  src_addr[0] = RSTEN;    /* set RESET Enable */
  SSP_Send( portNum, (uint8_t *)src_addr, 1 );
  GPIOSetBitValue( PORT0, 2, 1 );
  for ( i = 0; i < 0x80; i++ ); /* delay */
  GPIOSetBitValue( PORT0, 2, 0 );
  src_addr[0] = RST;      /* set RESET */
  SSP_Send( portNum, (uint8_t *)src_addr, 1 );
  GPIOSetBitValue( PORT0, 2, 1 );
  for ( i = 0; i < 0x80; i++ ); /* delay */
  ...
  SSP_Receive( portNum, (uint8_t *)dest_addr, 1 );
  GPIOSetBitValue( PORT0, 2, 1 );
  if ( dest_addr[0] & (RDSR_WEN|RDSR_BUSY) != RDSR_WEN )
  /* bit 7 to 0 is ready, bit 1 to 1 is write enable */
  {
    while ( 1 );
  }
  ...
  GPIOSetBitValue( PORT0, 2, 0 );
  src_addr[0] = WBPR;
//  src_addr[1] = 0x40;       /* Make device SST26xx32 0x0000~0x6000 unprotected.  */
//  src_addr[2] = 0x42;
//  src_addr[3] = 0x44;
  src_addr[1] = 0x20;       /* Make device SST26xx16 0x0000~0x6000 unprotected.  */
  src_addr[2] = 0x22;
  src_addr[3] = 0x24;
  SSP_Send( portNum, (uint8_t *)src_addr, 4 );
  GPIOSetBitValue( PORT0, 2, 1 );
  ...
  for ( i = 0; i < 0x80; i++ ); /* delay minimum 250ns */
  GPIOSetBitValue( PORT0, 2, 0 );
  src_addr[0] = SST_CHIP_ERASE; /* Chip erase on SST26xxx. */
  SSP_Send( portNum, (uint8_t *)src_addr, 1 );
  GPIOSetBitValue( PORT0, 2, 1 );
  ...
  while ( timeout < MAX_TIMEOUT )
  {
    ...
    SSP_Receive( portNum, (uint8_t *)dest_addr, 1 );
    GPIOSetBitValue( PORT0, 2, 1 );
    if ( (dest_addr[0] & RDSR_BUSY) == 0x00 ) /* bit 7 to 0 is ready */
    {
    break;
    }
    timeout++;
...
int main (void)
{
  ...
  within SSP peripheral or communicate with a serial Flash. */
#if LOOPBACK_MODE
  SSP_LoopbackTest( SSP_NUM );
#else
#if 0
  ATM25_SFlashTest( SSP_NUM );
#endif
#if 1
  SST26_SFlashTest( SSP_NUM );
#endif
  /* If SW CLK is used as SSP clock, change the setting before
  serial flash test, restore after the test. */
  ...
  for ( i = 0; i < SSP_BUFSIZE; i++ )
#else
  /* for Serial flash test, verifying, ignore the difference in the first
