/****************************************************************************
 *   $Id: gpio.c 28 2014-01-27 14:45:13Z danielru $
    ...
    uint32_t sense, uint32_t event )
{
  ...
  case CHANNEL1:
    if ( portNum )
    {
      LPC_SYSCON->PINSEL[1] = bitPosi + 32; 
    }
    else
    {
  ...
  case CHANNEL2:
    if ( portNum )
    {
      LPC_SYSCON->PINSEL[2] = bitPosi + 32; 
    }
    else
    {
  ...
  case CHANNEL3:
    if ( portNum )
    {
      LPC_SYSCON->PINSEL[3] = bitPosi + 32; 
    }
    else
    {
  ...
  case CHANNEL4:
    if ( portNum )
    {
      LPC_SYSCON->PINSEL[4] = bitPosi + 32; 
    }
    else
    {
  ...
  case CHANNEL5:
    if ( portNum )
    {
      LPC_SYSCON->PINSEL[5] = bitPosi + 32; 
    }
    else
    {
  ...
  case CHANNEL6:
    if ( portNum )
    {
      LPC_SYSCON->PINSEL[6] = bitPosi + 32; 
    }
    else
    {
  ...
  case CHANNEL7:
    if ( portNum )
    {
      LPC_SYSCON->PINSEL[7] = bitPosi + 32; 
    }
    else
    {
    ...
    uint32_t sense, uint32_t *eventPattern )
{
  ...
  case GROUP0:
    ...
    if (logic == 0)
    {
      LPC_GPIO_GROUP_INT0->CTRL &= ~(0x1<<1);
    }
    else
    {
      LPC_GPIO_GROUP_INT0->CTRL |= (0x1<<1);
    ...
    LPC_GPIO_GROUP_INT0->PORT_POL[0] = eventPattern[0];
    LPC_GPIO_GROUP_INT0->PORT_ENA[0] = bitPattern[0];
    LPC_GPIO_GROUP_INT0->PORT_POL[1] = eventPattern[1];
    LPC_GPIO_GROUP_INT0->PORT_ENA[1] = bitPattern[1];
  ...
  case GROUP1:
    ...
    if (logic == 0)
    {
      LPC_GPIO_GROUP_INT1->CTRL &= ~(0x1<<1);
    }
    else
    {
      LPC_GPIO_GROUP_INT1->CTRL |= (0x1<<1);
    }
    LPC_GPIO_GROUP_INT1->PORT_POL[0] = eventPattern[0];
    LPC_GPIO_GROUP_INT1->PORT_ENA[0] = bitPattern[0];
    LPC_GPIO_GROUP_INT1->PORT_POL[1] = eventPattern[1];
    LPC_GPIO_GROUP_INT1->PORT_ENA[1] = bitPattern[1];
    /* as soon as enabled, an edge may be generated       */
    /* clear interrupt flag and NVIC pending interrupt to */
    /* workaround the potential edge generated as enabled */
