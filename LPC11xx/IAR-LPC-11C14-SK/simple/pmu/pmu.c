...
/*****************************************************************************
** Function name:		WAKE_UP7_IRQHandler
**
...
void PMU_Init( void )
{
  ...
  /* use port0_7 as wakeup source, i/o pin */
  LPC_IOCON->PIO0_7 &= ~0x07;	
  LPC_IOCON->PIO0_7 |= 0x20;	
  GPIOSetDir( PORT0, 1, 0 );	/* Input P0.7 */
  /* Only edge trigger. activation polarity on P0.7 is FALLING EDGE. */
  LPC_SYSCON->STARTAPRP0 = 0x00000000;
  /* Clear all wakeup source */
  LPC_SYSCON->STARTRSRP0CLR = 0xFFFFFFFF;
  /* Enable Port 0.7 as wakeup source. */
