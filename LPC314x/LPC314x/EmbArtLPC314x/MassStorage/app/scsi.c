...
void UsbBotRstInterface (void)
{
  ...
  BotState = BotWaitCbw;
  Lun = 0;
  for(Int32U i = 0;SCSI_LUN_NUMB > i; ++i)
  {
    ScsiDrv[i].Message[0] = LunResetReqMsg;
    pScsiMessage[i]       = ScsiDrv[Lun].Message;
  }

  BotStatus.Flags &= 1UL;
  if(BotStatus.PhaseError)
  {
    USB_SetStallEP(BulkOutEp,1);
    USB_SetStallEP(BulkInEp,1);
  }
  else
  {
    USB_SetStallEP(BulkInEp,0);
    USB_SetStallEP(BulkOutEp,0);
    // Clear all flags
...
void BotOutEP (USB_Endpoint_t EP)
{
  ...
  // Something is wrong
  ScsiCmdError(ScsiFatalError,ScsiStallIn | ScsiStallOut);
}
...
void ScsiCommImpl (void)
{
  if(ScsiCbwValid())
  {
    ...
    default:
      ...
      {
        ScsiCmdError(ScsiUnknowCommand,0);
      }
    }
    return;
  }
...
pSpc3RequestSense_t pRequestSense = (pSpc3RequestSense_t)Cbw.CBWCB;
  if(0 == pRequestSense->AllocationLenght)
  {
    BotSendStatus();
    return;
  }
  ...
  if (Cbw.dCBWDataTransferLength >= pRequestSense->AllocationLenght)
  {
    // clear buffer
    memset(&Req2Sense,0,sizeof(Spc3RequestSenseResponse_t));
    if ((pRequestSense->Control.Control == 0) && !pRequestSense->DESC)
    {
      Req2Sense.ResponceCode = 0x70;
      Req2Sense.Valid = TRUE;
      switch (ScsiDrv[Lun].Status)
      {
      case ScsiMediamNotReady:
        Req2Sense.Ascq = LogicalUnitNotReadyCauseNotReportable >> 8;
        Req2Sense.Asc  = LogicalUnitNotReadyCauseNotReportable;
        Req2Sense.SenseKey  = NotReady;
        break;
      case ScsiInvalidCbd:
        Req2Sense.Ascq = InvalidFieldInCdb >> 8;
        Req2Sense.Asc  = InvalidFieldInCdb;
        Req2Sense.SenseKey   = IllegalRequest;
        break;
      case ScsiUnknowCommand:
        Req2Sense.Ascq = InvalidCommandOperationCode >> 8;
        Req2Sense.Asc  = InvalidCommandOperationCode;
        Req2Sense.SenseKey   = IllegalRequest;
        break;
      case ScsiMediaNotPresent:
        Req2Sense.Ascq = MediumNotPresent >> 8;
        Req2Sense.Asc  = MediumNotPresent;
        Req2Sense.SenseKey   = NotReady;
        break;
      case ScsiMiscompare:
        Req2Sense.Ascq = MiscompareDuringVerifyOperation >> 8;
        Req2Sense.Asc  = MiscompareDuringVerifyOperation;
        Req2Sense.SenseKey   = Miscompare;
        break;
      case ScsiMediaChanged:
        Req2Sense.Ascq = NotReadyToReadyChangeMediumMayHaveChanged >> 8;
        Req2Sense.Asc  = NotReadyToReadyChangeMediumMayHaveChanged;
        Req2Sense.SenseKey   = UnitAttention;
        break;
      default:
        Req2Sense.Ascq = NoAdditionalSenseInformation >> 8;
        Req2Sense.Asc  = NoAdditionalSenseInformation;
        Req2Sense.SenseKey   = NoSense;
        break;
      }
      Req2Sense.AddSenseKeyLength = sizeof(Spc3RequestSenseResponse_t) - 8;
      // Clear condition code info
      ScsiDrv[Lun].Status = ScsiCommandNoKey;
      // Send Status after data packet
      if(BotPass != BotSendDataInit((Int8U *)&Req2Sense,
...
pSpc3ModeSelect6_t pModeSelect = (pSpc3ModeSelect6_t)Cbw.CBWCB;
  if(0 == pModeSelect->ParameterListLenght)
  {
    BotSendStatus();
    return;
  }
  ...
  if (Cbw.dCBWDataTransferLength >= pModeSelect->ParameterListLenght)
  {
    ScsiCmdStatus(ScsiInvalidCbd);
  }
  else
...
pSpc3ModeSense6_t pModeSense = (pSpc3ModeSense6_t)Cbw.CBWCB;
  ...
  if (Cbw.dCBWDataTransferLength >= pModeSense->AllocationLenght)
  {
    if (   ( pModeSense->PC          == 0)
...
pSpc3ModeSense6Response_t pModeSense = (pSpc3ModeSense6Response_t)&Cbw;
  ...
  if(BotPass != BotSendDataInit((Int8U *)pModeSense,
...
Int32U AllocationLenght =
  ...
  if (Cbw.dCBWDataTransferLength >= AllocationLenght)
  {
...
{
  if(BotPass != BotSendDataInit(pData,MIN(Csw.dCSWDataResidue,Size),TRUE))
  {
    BotStatus.BotStatus = TRUE;
    BotInEpStall();
    ScsiDrv[Lun].Message[0] = LunResetReqMsg;
...
void ScsiReadCapacity10Imp (void)
{
  ...
  if (Cbw.dCBWDataTransferLength >= sizeof(Sbc2ReadCapacity10Response_t))
  {
...
pSbc2ReadCapacity10Response_t pReadCapacity = (pSbc2ReadCapacity10Response_t)&Cbw;
  ...
  if(BotPass != BotSendDataInit((Int8U *)pReadCapacity,
...
{
  if(BotPass != BotSendDataInit(pData,Size,DataComplete))
  {
    BotStatus.BotStatus = TRUE;
    BotInEpStall();
    ScsiDrv[Lun].Message[0] = LunResetReqMsg;
...
void ScsiWriteData(pInt8U pData, Int32U Size, Boolean DataComplete)
{
  ...
  {
    if(BotPass != BotReceiveDataInit(pData,Size,DataComplete))
    {
      ScsiCmdStatus(ScsiFatalError);
      BotSendStatus();
      BotOutEpStall();
...
Int32U AllocationLength = (pFormatCapacity->AllocationLenght[0]<<8) +\
  ...
  if(BotPass != BotSendDataInit(pData,MIN(Size,AllocationLength),TRUE))
  {
    BotStatus.BotStatus = TRUE;
    BotInEpStall();
    ScsiDrv[Lun].Message[0] = LunResetReqMsg;
