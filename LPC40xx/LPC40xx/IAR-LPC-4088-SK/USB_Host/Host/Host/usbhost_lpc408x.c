...
void  Host_Init (void)
{
    ...
    LPC_USB->OTGClkCtrl  = 0x00000009;                        /* Enable USB host clock                      */

    while ((LPC_USB->OTGClkSt & 0x00000001) == 0) {
    }
    
    LPC_USB->StCtrl = 0x00000001;                 /**/
                                   
    LPC_IOCON->P0_31 = 0x1;                       /* P0[31] = USB_D+2                         */
                                                              
    LPC_IOCON->P1_30 = 0x1A1;                     /* P1[30] = USB_PWRD2                       */
    LPC_IOCON->P1_31 = 0x1A1;                     /* P1[31] = USB_OVRCR2                      */
    LPC_IOCON->P0_12 = 0x1A1;                     /* P0[12] = USB_PPWR2                       */
    ...
    NVIC_EnableIRQ(USB_IRQn);                                 /* Enable the USB interrupt source            */
...
void USB_IRQHandler(void)
{
    ...
    if ((LPC_SC->USBIntSt & 0x00000008) > 0) {
