...
static uint32_t BlockStart, BlockNum;
...
uint32_t Temp;
  ...
  // Get a message
  uint32_t Message   = *pScsiMessage[LunInd];
  uint32_t * pMessage = pScsiMessage[LunInd]+1;
  ...
  case LunCommandDecode:
    ...
    case LunInquiryReqMsg:
      memcpy(Lun0Buffer,Lun->DiskInquiry,Lun->DiskInquirySize);
      ScsiInquiryData(Lun0Buffer,Lun->DiskInquirySize);
      break;
