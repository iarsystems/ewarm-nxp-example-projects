...
const uint8_t LunMax = SCSI_LUN_NUMB-1;
 ...
 *
 *************************************************************************/
BotToUserStatus_t BotReceiveDataInit (uint8_t * pData, uint32_t DataSize, Boolean DataComplete)
{
#if __CORE__ < 7
uint32_t Save;
#endif // __CORE__ < 7
 ...
 *
 *************************************************************************/
BotToUserStatus_t BotSendDataInit (const uint8_t * pData, uint32_t DataSize, Boolean DataComplete)
{
#if __CORE__ < 7
uint32_t Save;
#endif // __CORE__ < 7
...
pSpc3RequestSenseResponse_t pRequestSenseResponse;
      ...
      (Cbw.dCBWDataTransferLength >= pRequestSense->AllocationLenght))
  {
    ...
    if ((pRequestSense->Control.Control == 0) && !pRequestSense->DESC)
    {
      ...
      BotSendDataInit((uint8_t *)pRequestSenseResponse,
...
pSpc3ModeSense6Response_t pModeSense = (pSpc3ModeSense6Response_t)&Cbw;
  ...
  BotSendDataInit((uint8_t *)pModeSense,
...
uint32_t AllocationLenght =
  ((uint32_t)pInquiry->AllocationLenght[0]<<8) + pInquiry->AllocationLenght[1];
  if ((Cbw.bmCBWFlags.Dir == BotDataFormDevToHost) &&
...
pSbc2ReadCapacity10Response_t pReadCapacity = (pSbc2ReadCapacity10Response_t)&Cbw;
  ...
  BotSendDataInit((uint8_t *)pReadCapacity,
...
pSbc2Read10_t pRead = (pSbc2Read10_t)Cbw.CBWCB;
  if (Cbw.bmCBWFlags.Dir == BotDataFormDevToHost)
  {
    ...
    else
    {
      // Clear condition code info
      ScsiDrv[Lun].Status = ScsiCommandNoKey;
      ScsiDrv[Lun].Message[2] = ((uint32_t)pRead->TransferLength[0] << 8) + pRead->TransferLength[1];
      if(ScsiDrv[Lun].Message[2] != 0)
      {
        ScsiDrv[Lun].Message[0] = LunRead10ReqMsg;
        ScsiDrv[Lun].Message[1] = ((uint32_t)pRead->LogicalBockAddress[0] << 24) +\
                                  ((uint32_t)pRead->LogicalBockAddress[1] << 16) +\
                                  ((uint32_t)pRead->LogicalBockAddress[2] << 8 ) +\
...
pSbc2Write10_t pWrite = (pSbc2Write10_t)Cbw.CBWCB;
  if (Cbw.bmCBWFlags.Dir == BotDataFormHostToDev)
  {
    ...
    else
    {
      // Clear condition code info
      ScsiDrv[Lun].Status = ScsiCommandNoKey;
      ScsiDrv[Lun].Message[2] = ((uint32_t)pWrite->TransferLength[0] << 8) + pWrite->TransferLength[1];
      if(ScsiDrv[Lun].Message[2] != 0)
      {
        ScsiDrv[Lun].Message[0] = LunWrite10ReqMsg;
        ScsiDrv[Lun].Message[1] = ((uint32_t)pWrite->LogicalBockAddress[0] << 24) +\
                                  ((uint32_t)pWrite->LogicalBockAddress[1] << 16) +\
                                  ((uint32_t)pWrite->LogicalBockAddress[2] << 8 ) +\
...
pSbc2Verify10_t pVerify = (pSbc2Verify10_t)Cbw.CBWCB;
  if (Cbw.bmCBWFlags.Dir == BotDataFormHostToDev)
  {
    ...
    else
    {
      // Clear condition code info
      ScsiDrv[Lun].Status = ScsiCommandNoKey;
      ScsiDrv[Lun].Message[2] = ((uint32_t)pVerify->TransferLength[0] << 8) + pVerify->TransferLength[1];
      if(ScsiDrv[Lun].Message[2] != 0)
      {
        ScsiDrv[Lun].Message[1] = ((uint32_t)pVerify->LogicalBockAddress[0] << 24) +\
                                  ((uint32_t)pVerify->LogicalBockAddress[1] << 16) +\
                                  ((uint32_t)pVerify->LogicalBockAddress[2] << 8 ) +\
...
uint32_t AllocationLength = (pFormatCapacity->AllocationLenght[0]<<8) +\
