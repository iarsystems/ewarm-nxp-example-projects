...
===============
  ...
  1) Open the USB_Host work space.
  ...
  3) If you previously haven't run any of the IAR-LPC4088-SK examples
     ...
     - Switch back to the USB_Host project.

  4) Build the USB_Host example. Press CTRL+D or use Download and
     Debug button to start a debug session.
 
  5) Press F5 or use the GO button to run the example.
  
  6) Plug a USB flash drive into IAR-LPC4088-SK USB Host port. If there is 
     MSREAD.TXT file in the USB flash drive root directory, the USB Host demo 
     will copy its content to a MSWRITE.TXT file.