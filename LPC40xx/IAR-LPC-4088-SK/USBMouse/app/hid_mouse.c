...
uint8_t DataBuffer[sizeof(UsbHidDescriptor_t)];
#pragma data_alignment=4
...
void HidCnfgInit (pUsbDevCtrl_t pDev)
{
  if (pDev != NULL && pDev->Configuration)
  {
    ...
    RelativeData0 = FALSE;
  #if HID_IDLE_SUPP > 0
    for(uint32_t i = (HID_ID_NUMB?1:0); i < HID_ID_NUMB+1; i++)
    {
      HidIdle[i].Dly  = \
      HidIdle[i].Cnt  = 0;
    }
...
UsbCommStatus_t UsbClassHidRequest (pUsbSetupPacket_t pSetup)
{
  ...
  case HID_GET_IDLE:
    if (pSetup->wIndex.Word == HID_INTERFACE_0)
    {
      ...
      {
        USB_IO_Data(CTRL_ENP_IN,
                   (uint8_t *)&HidIdle[pSetup->wValue.Lo].Dly,
  ...
  case HID_SET_IDLE:
    if (pSetup->wIndex.Word == HID_INTERFACE_0)
    {
      if(pSetup->wValue.Lo <= HID_ID_NUMB)
      {
        ...
        {
          for(uint32_t i = 1; i < HID_ID_NUMB+1; i++)
          {
 ...
 * Parameters:  int8_t X, int8_t Y, uint8_t Buttons
 *
 * Return: none
 *
 ...
 *
 *************************************************************************/
void HidMouseSendReport (int8_t X, int8_t Y, uint8_t Buttons)
{
  MouseReport.X = X;
  MouseReport.Y = Y;
 ...
 * Parameters:  Boolean Ctrl, uint8_t ID
 *
 * Return: none
 *
 ...
 *************************************************************************/
static
Boolean HidSendInReport (Boolean Ctrl, uint8_t ID)
{
#if __CORE__ < 7
uint32_t Save;
#endif // __CORE__ < 7
  ...
  case HID_MOUSE_ID:
    ...
    USB_IO_Data((Ctrl?CTRL_ENP_IN:ReportEpHid),
              #if HID_ID_NUMB > 0
                (uint8_t *)(((uint8_t *)&MouseReport) + ((HidProtocol == ReportProtocol)?0:1)),     // first byte is ID
              #else
                (uint8_t *)&MouseReport,
              #endif // HID_ID_NUMB > 0
