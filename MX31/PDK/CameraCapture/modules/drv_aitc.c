...
Int32U NIMask = NIMASK;         //Save current prioriry level
Int32U IndexSatusReg = NIVECSR; //Read Interrupt indedx and Status

  NIMASK = IndexSatusReg & 0xFF;  //Disable Interrupts with currnet and lower priority
  Dummy = NIMASK ;

  if ((-1UL) != IndexSatusReg)
  {
    __enable_irq();
    
    ((IntrFunc_t)(*(&VECTOR0 + ((IndexSatusReg >> 16) & 0x3F))))(); //Call installed interrupt service routine
  ...
  NIMASK = NIMask;                           // Priority Level Restore
...
void AITC_Init(pAITC_Ctrl_t pAITC_Ctrl)
{
  ...
  //AVIC will not accelerate nIRQ servicing i.e. complete software control
  INTCNTL_bit.NM = 0;
  //Priority Sticky Control
  INTCNTL_bit.ABFEN = pAITC_Ctrl->PrioritySticky;
...
void AITC_SetupIntr(IntrFunc_t pIntrSub, Boolean FastIntr,
                    Int32U IntSource, Int32U Priority)
{
IntrFunc_t * Vector = (IntrFunc_t *)(&VECTOR0+IntSource);
  // Sanitary check
  if(IntSource > 63)
  {
    return;
  ...
  if(!FastIntr)
  {
    AITC_SetIrqPriority(IntSource,Priority);
  }

  *Vector = pIntrSub;
  AITC_SetIntType(IntSource,FastIntr);
}
