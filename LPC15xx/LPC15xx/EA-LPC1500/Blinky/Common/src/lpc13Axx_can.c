/****************************************************************************
 *   $Id: lpc13Axx_can.c 28 2014-01-27 14:45:13Z danielru $
 *   Project: NXP LPC13Axx CAN example
 *
 *   Description:
...
void CAN_Init( uint32_t CANBitClk )
{
  ...
  NVIC_DisableIRQ(CAN_IRQn);
	...
	LPC_SYSCON->SYSAHBCLKCTRL1 |= EN1_CAN;
  LPC_SYSCON->PRESETCTRL1 |= EN1_CAN;
  LPC_SYSCON->PRESETCTRL1 &= ~EN1_CAN;

  /* Conect CAN Pins using SWM, CAN_TX(bit 8~15) and CAN_RX(bit 16~23) */
  regVal = LPC_SWM->PINASSIGN6 & 0xFF0000FF;
	regVal |= (0x48 << 8) | (0x49 << 16);			/* 2.8 is CAN TXD, 2.9 is CAN RXD. */
	LPC_SWM->PINASSIGN6 = regVal; 
