...
#define SYSPLLCTRL_Val        0x00000046              // Reset: 0x000
#define SYSPLLCLKSEL_Val      0x00000000              // Reset: 0x000
#define MAINCLKSELA_Val       0x00000003              // Reset: 0x000
...
#define __DIVSEL   (((WDTOSCCTRL_Val & 0x1F) << 1) + 2)
  ...
  /* main clock calculation */
    ...
    #define __MAIN_CLOCK             (__SYS_OSC_CLK)
	  ...
	  #if (__FREQSEL ==  0)
      #error "MAINCLKSELA: WDT Oscillator selected but FREQSEL is undefined!"
    #else
  ...
  wdt_osc /= ((LPC_SYSCON->WDTOSCCTRL & 0x1F) << 1) + 2;
    ...
    switch (LPC_SYSCON->MAINCLKSELA & 0x03) {
      ...
      case 3:                             /* System PLL Clock Out               */
        SystemCoreClock = __RING_OSC_CLK;
        break;			
      }
  }
	else if ( LPC_SYSCON->MAINCLKSELB == 1 ) {
    SystemCoreClock = __SYS_PLLCLKIN;
	}
	else if ( LPC_SYSCON->MAINCLKSELB == 2 ) {
    switch (LPC_SYSCON->SYSPLLCLKSEL & 0x03) {
      case 0:                       /* Internal RC oscillator             */
        SystemCoreClock = __IRC_OSC_CLK * ((LPC_SYSCON->SYSPLLCTRL & 0x03F) + 1);
        break;
      case 1:                       /* System oscillator                  */
        SystemCoreClock = __SYS_OSC_CLK * ((LPC_SYSCON->SYSPLLCTRL & 0x03F) + 1);
        break;
      case 2:                       /* Reserved                           */
      case 3:                       /* Reserved                           */
        SystemCoreClock = 0;
	...
	else if ( LPC_SYSCON->MAINCLKSELB == 3 ) {
    SystemCoreClock = __ULP_32K_OSC_CLK;
	}
...
void SystemInit (void) {
  ...
  LPC_SYSCON->PDRUNCFG     &= ~PDEN_WDT_OSC;      /* Power-up WDT OSC Clock   */
  for (i = 0; i < 200; i++) __NOP();
#endif

  LPC_SYSCON->MAINCLKSELB   = MAINCLKSELB_Val;     /* Select PLL Clock Output  */
  LPC_SYSCON->SYSAHBCLKDIV  = SYSAHBCLKDIV_Val;

#if ((USBCLKDIV_Val & 0x1FF) != 0)                /* USB clock is used        */
  LPC_SYSCON->PDRUNCFG     &= ~PDEN_USB_PHY;      /* Power-up USB PHY         */

  /* Regardless USB PLL is used as USB clock or not, USB PLL needs to be configured. */
  LPC_SYSCON->PDRUNCFG     &= ~PDEN_USB_PLL;      /* Power-up USB PLL         */
