/**********************************************************************
* $Id: lpc13Axx_uart.c 28 2014-01-27 14:45:13Z danielru $		lpc13Axx_uart.c		2011-06-02
*//**
* @file		lpc13Axx_uart.c
* @brief	Contains all functions support for UART firmware library on lpc13Axx
* @version	1.0
* @date		02. June. 2011
...
** 
*****************************************************************************/
void UART_Handler( LPC_UART_TypeDef *UARTx )
{
...
** 
*****************************************************************************/
void FlowControlInit( LPC_UART_TypeDef *UARTx )
{			
...
** 
*****************************************************************************/
void UARTClock_Init( LPC_UART_TypeDef *UARTx )
{
  LPC_SYSCON->FRGCLKDIV = 1;     /* divided by 1 */
	...
	if (UARTx == LPC_UART0)
	{
		...
		LPC_SYSCON->PRESETCTRL1 |= EN1_UART0;
		LPC_SYSCON->PRESETCTRL1 &= ~EN1_UART0;
	}
	if (UARTx == LPC_UART1)
	{
		...
		/* Peripheral reset control to UART, a "1" bring it out of reset. */
		LPC_SYSCON->PRESETCTRL1 |= EN1_UART1;
		LPC_SYSCON->PRESETCTRL1 &= ~EN1_UART1;
	}
	if (UARTx == LPC_UART2)
	{
		...
		/* Enable UART clock */
		LPC_SYSCON->SYSAHBCLKCTRL1 |= EN1_UART2;
		/* Peripheral reset control to UART, a "1" bring it out of reset. */
		LPC_SYSCON->PRESETCTRL1 |= EN1_UART2;
		LPC_SYSCON->PRESETCTRL1 &= ~EN1_UART2;
	}
	return;	
}
...
** 
*****************************************************************************/
void UARTInit(LPC_UART_TypeDef *UARTx, uint32_t baudrate)
{
	uint32_t UARTSysClk;
	uint32_t subVal;
  ...
  UARTTxEmpty = 1;
	...
	UARTSysClk = SystemCoreClock/LPC_SYSCON->FRGCLKDIV;
  ...
  UARTx->CTRL = ADDR_DET;
	...
	else
	{
		...
		*/
		...
		/* In FRGCTRL, bit 15:8 is ADD, bit 7:0 is SUB. */ 
		subVal = 0xFF;
		LPC_SYSCON->FRGCTRL = (((((UARTSysClk / 16) * (subVal + 1)) / (baudrate * (UARTx->BRG + 1))) - (subVal + 1))<<8)|subVal;
	}
...
** 
*****************************************************************************/
void UARTSend(LPC_UART_TypeDef *UARTx, uint8_t *BufferPtr, uint32_t Length)
{
#if ADDR_DETECT_EN
  uint32_t flag = 0;
#endif
...
** 
*****************************************************************************/
void USARTInit(LPC_UART_TypeDef *UARTx, uint32_t baudrate, uint32_t mode)
{
  UARTTxEmpty = 1;
  NVIC_DisableIRQ(UART0_IRQn);
...
//  UARTx->CFG = DATA_LENG_8|PARITY_ODD|STOP_BIT_1; /* 8 bits, odd Parity, 1 Stop bit */
  ...
  UARTx->BRG = (SystemCoreClock/LPC_SYSCON->FRGCLKDIV)/baudrate-1;	/*baud rate */
  UARTx->STAT = CTS_DELTA | DELTA_RXBRK;		/* Clear all status bits. */		
