/**********************************************************************
* $Id: Uart_Dma.c 28 2014-01-27 14:45:13Z danielru $		Uart_Dma.c			2011-06-02
*//**
* @file		Uart_Dma.c
...
"MCU LC43xx - ARM Cortex-M4+M0 \n\r\t "
"UART1 - 9600bps \n\r"
" This is a long string. It transferred in to DMA memory and transmit through Tx line \n\r"
" on UART1 peripheral. To use UART with DMA mode, FIFO function must be enabled \n\r";
...
int c_entry(void)
{
    ...
    while (1)
    {
        ...
        while ((Channel1_TC == 0) && (Channel1_Err == 0)){
			...
			if (*rx_char != 0)
			{
				UART_Send((LPC_USARTn_Type *)LPC_UART1, rx_char, 1, BLOCKING);
				rx_char++;
			}
        }
    }
