...
/************************************************************/
	...
	#define IRQ_LOCK_KEY  
	#define _lockInts()		__set_BASEPRI((MASTER_IPC_PRIORITY << (8 - __NVIC_PRIO_BITS) & 0xFF ))
...
{	
	...
	NVIC_SetPriority((IRQn_Type)MASTER_IRQn, SLAVE_IPC_PRIORITY);
...
{	
	...
	NVIC_SetPriority((IRQn_Type)SLAVE_IRQn, MASTER_IPC_PRIORITY);
