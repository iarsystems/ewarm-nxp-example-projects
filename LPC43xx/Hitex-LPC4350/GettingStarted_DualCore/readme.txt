...
===========
  ...
  embeds the CortexM0 application image to the program code. When the application
...
=============
  ...
  one step left before the LPC4350 can boot from external flash. That final
  step is to configure the boot mode using the boot mode jumpers.
  ...
  Note: Boot config pin P2_9 must be pulled down to GND via 10k resistor
   ...
   The GettingStarted_DualCore application is downloaded to NOR, SPI Flash (SPIFI)
