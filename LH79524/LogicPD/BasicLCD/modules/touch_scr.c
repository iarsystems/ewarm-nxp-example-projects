...
volatile Int32U Dummy;
  ...
  MUXCTL25_bit.AN2 = 0; // AN2/LL/Y+
  MUXCTL25_bit.AN3 = 0; // AN3/LR/Y-
  // Clock set
  PCLKSEL1_bit.ADC = 1; // System Clock Oscillator frequency
  ADCPRE = 0;           // MAIN_OSC_FREQ/1
  PCLKCTRL1_bit.ADC = 0;// Enables the ADC Clock
  ...
  VIC_SetVectoredIRQ(TouchScrPen_Handler,PenTouchSlot,PENIRQ_IRQ);
  VIC_EnableInt(1UL << PENIRQ_IRQ);
  VIC_SetVectoredIRQ(TouchScrEOC_Handler,EndOfConvSlot,TSCIRQ_IRQ);
  VIC_EnableInt(1UL << TSCIRQ_IRQ);
