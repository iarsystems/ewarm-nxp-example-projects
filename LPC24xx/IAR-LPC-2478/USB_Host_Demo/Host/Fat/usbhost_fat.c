...
void        FAT_File_SetAttr    (         FILE_ENTRY *entry,
                                          USB_INT08S  attr);
static
void        FAT_FreeClusters    (         USB_INT32S firstClust);
/*
**************************************************************************************************************
*                                              GLOBAL VARIABLES
**************************************************************************************************************
                          ...
                          USB_INT08U   flags)
{
    ...
    if (flags == RDONLY) {                       /* Search for a file. If it doesn't exist, don't create it */
        rc2 = FAT_FindEntry(file_name, entry);
        if (rc2 == MATCH_FOUND) {
        #ifdef FAT_TIME_DATE_ENA
        ...
        rc2 = FAT_CreateEntry(file_name, entry);
        if(rc2 >= 0) {
            entry->Status = flags;
            if (entry->FileSize > 0)
            {
                if (flags == APPEND) {          // set CurrClus and CurrClusOffset
                    entry->CurrClus = FAT_GetEndClus(entry->CurrClus);
                    entry->CurrClusOffset = entry->FileSize % FAT_Desc.BytsPerClus;
                } else {                        // release clusters
                    FAT_FreeClusters(entry->CurrClus);
                    entry->FileSize = 0;
                }
            }
          rc = (USB_INT32U)entry;
                                 ...
                                 USB_INT32U   num_bytes)
{
    ...
    USB_INT32U   total_bytes_read   = 0;         /* Total bytes read                                        */
    ...
    USB_INT32U   remainder;

    entry = (FILE_ENTRY *)fd;                    /* Get file entry from file descriptor                     */
    remainder = entry->FileSize - entry->FilePosition;

    if (remainder == 0) {
        return (0);
    }
    if (num_bytes <= remainder) {
        total_bytes_to_read = num_bytes;
    } else {
        total_bytes_to_read = remainder;
    }
    do {
        ...
        if (next_clus == 0) {                             /* If the current cluster is the last cluster     */
            ...
            if (total_bytes_to_read > FAT_Desc.BytsPerClus - entry->CurrClusOffset) {   // FAT error
                total_bytes_to_read = FAT_Desc.BytsPerClus - entry->CurrClusOffset;
            }
        ...
        entry->FilePosition += bytes_read;
                                             /* If the cluster offset reaches end of the cluster, make it 0 */
        if (entry->CurrClusOffset + bytes_read == FAT_Desc.BytsPerClus && next_clus > 0) {
            entry->CurrClusOffset = 0;
            entry->CurrClus = next_clus;
        } else {
...
USB_INT32U  i;
  ...
  do
  {
    if(NULL == (buf = FAT_GetNexRootEntry(buf,&sec_num)))
    {
      ...
      {
        IOC_Write(sec_num + i, 1, (USB_INT08U *)FATBuffer);
      }
                  ...
                  FILE_ENTRY  *entry)
{
    ...
    for (idx = 0; idx < 8; idx++) {        /* Fill the first eight characters of the entry with file name   */
        ...
        if (c == '.' || c == 0) {
            ...
            FATBuffer[entry->EntrySecOffset + idx] = c;
        }
    }
    if (*ent_name_given == '.')
            ent_name_given++;
    ...
    FATBuffer[entry->EntrySecOffset + 11] = FILE_ATTR_ARCH;
...
static USB_INT32U   clus_sec_num;               // first sector of the cluster
static USB_INT32U   sec_num;
static USB_INT32U   secPerClus;
  ...
  if(NULL == buf)
  {
    if(FAT_32 == FAT_Desc.FATType)
    {
      ...
      secPerClus = FAT_Desc.SecPerClus;
      clus_num = FAT_Desc.RootClus;
    }
    else
    {
      secPerClus = FAT_Desc.RootDirSec;
    }
    clus_sec_num = sec_num = FAT_Desc.RootDirStartSec;                /* For all the sectors in root directory       */
  ...
  if(sec_num >= (clus_sec_num + secPerClus))
  {
    if(FAT_32 == FAT_Desc.FATType)
    {
      if((clus_num = FAT_GetNextClus(clus_num)))
      {
        clus_sec_num = sec_num = ((clus_num-2) * FAT_Desc.SecPerClus)+ FAT_Desc.FirstDataSec;
//      secPerClus = FAT_Desc.SecPerClus;
        IOC_Read(++sec_num, 1, (USB_INT08U *)FATBuffer);              /* Read one sector                             */
        if(NULL != p_sec_num)
        {
...
USB_INT32U temp;
  ...
  if(NextFreeClus)
  {
    ...
    if (ReadLE32U(&FATBuffer[492]) > NextFreeClus)
    {
...
//*************************************************
{
    ...
    FAT_UpdateFSInfo(clust_count, firstClust);
}
