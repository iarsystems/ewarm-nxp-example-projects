...
**						Match3 is used to trigger the update of PWM
**
**
** parameters:			None
** Returned value:		None
...
void init_mtimer0( DWORD timer_interval ) 
{
  ...
  /* Reset counter if Match 3 counter equals to timer counter. */
  MTIM0_MCR = (0x01<<6);		/* For PWM reg update on Match 3 */

  /* Set MATCH3 register */
//  MTIM0_MR3 = timer_interval/4 - 1;	/* For PWM shadow register update */
  MTIM0_MR3 = (20*1000)/PWM_CYCLE - 1;	/* For PWM shadow register update */

  /* match 3 will generate toggling. */
  MTIM0_EMR |= (0x03<<10);		/* For PWM shadow register update */
  return;
}
